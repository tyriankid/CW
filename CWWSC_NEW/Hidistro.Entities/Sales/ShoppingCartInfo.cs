namespace Hidistro.Entities.Sales
{
    ...
    public class ShoppingCartInfo
    {
        ...
        /// </summary>
        /// <returns></returns>
        public int GetPointPt()
        {
            int num = 0;
            SiteSettings masterSettings = SettingsManager.GetMasterSettings(true);
            if (((this.GetTotal() * this.TimesPoint) / masterSettings.PointsRatePt) > 2147483647M)
            {
                return 0x7fffffff;
            }
            if (masterSettings.PointsRatePt != 0M)
            {
                num = (int)Math.Round((decimal)((this.GetTotal() * this.TimesPoint) / masterSettings.PointsRatePt), 0);
            }
            return num;
        }
