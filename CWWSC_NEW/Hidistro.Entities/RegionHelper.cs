namespace Hidistro.Entities
{
    ...
    public static class RegionHelper
    {
        ...
        public static Dictionary<int, string> GetAllStreetProvinces(DataTable streetInfo)
        {
            //如果门店配送功能打开了,只展示已经绑定了街道的省市区
            Dictionary<int, string> dictionary = new Dictionary<int, string>();
            foreach (DataRow row in streetInfo.Rows)
            {
                XmlNodeList list = GetRegionDocument().SelectNodes("//province");
                foreach (XmlNode node in list)
                {
                    if (node.Attributes["id"].Value.ToInt() == GetTopRegionId(row["RegionCode"].ToInt()))
                    {
                        dictionary.Add(int.Parse(node.Attributes["id"].Value), node.Attributes["name"].Value);
                    }
                }
        ...
        {
            return GetChildList(string.Format("root/province[@id='{0}']", provinceId.ToString(CultureInfo.InvariantCulture)));
        }

        public static Dictionary<int, string> GetCountys(int cityId)
        {
            return GetChildList(string.Format("root/province/city[@id='{0}']", cityId.ToString(CultureInfo.InvariantCulture)));
        }
        ...
        public static string GetFullRegion(int currentRegionId, string separator)
        {
            ...
            //}
            for (XmlNode node2 = node.ParentNode; node2.Name != "root"; node2 = node2.ParentNode)
            {
                str = node2.Attributes["name"].Value + separator + str;
            }
            return str;
        ...
        public static Dictionary<int, string> GetProvinces(int regionId)
        {
            ...
            return GetChildList(string.Format("//province[@id='{0}']", regionId.ToString(CultureInfo.InvariantCulture)));
        }
