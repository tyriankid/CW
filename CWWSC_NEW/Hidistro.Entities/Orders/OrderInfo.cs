namespace Hidistro.Entities.Orders
{
    ...
    public class OrderInfo
    {
            ...
            if ((this.OrderStatus != Hidistro.Entities.Orders.OrderStatus.Finished) && (this.OrderStatus != Hidistro.Entities.Orders.OrderStatus.Closed))
            {
                switch (action)
                {
                    ...
                    case OrderActions.SELLER_FINISH_TRADE:
                        return ((this.OrderStatus == Hidistro.Entities.Orders.OrderStatus.SellerAlreadySent) || (this.OrderStatus == Hidistro.Entities.Orders.OrderStatus.ConfirmTakeGoods));//确认收货也允许订单设置为完成状态
        ...
        /// <summary>
        /// 计算除去了买一送一,优惠券等活动的订单总额
        /// </summary>
        /// <returns></returns>
        public decimal GetShowAmount()
        {
            decimal num = 0M;
            foreach (LineItemInfo info in this.LineItems.Values)
            {
                num += info.GetSubShowTotal();
            }
            return num;
        }
        ...
        public static string GetOrderStatusName(Hidistro.Entities.Orders.OrderStatus orderStatus)
        {
            switch (orderStatus)
            {
                ...
                case Hidistro.Entities.Orders.OrderStatus.ConfirmTakeGoods:
                    return "已收货";
        ...
        public static string GetOrderStatusNameService(Hidistro.Entities.Orders.OrderStatus orderStatus)
        {
            switch (orderStatus)
            {
                case Hidistro.Entities.Orders.OrderStatus.WaitBuyerPay:
        ...
        public int CutDownBuyId { get; set; }
        ...
        /// 订单是否提交到港口
        /// </summary>
        public int submitgk { get; set; }


        /// <summary>
        /// 订单是否提交到酷开
        /// </summary>
        public int submitkk { get; set; }
        ...
        /// <summary>
        /// 订单编号，自增长值，只用于调用创维订单接口传入的值，其它地方无用
        /// </summary>
        ...
        /// <summary>
        /// 获取商品描述，用于推送消息(此属性上限30字)
        /// </summary>
        public string ProductDescription
        {
            get
            {
                string itemDescription = this.OrderId;
                if (this.LineItems.Count() > 0)
                {
                    itemDescription = ((LineItemInfo)this.LineItems.First().Value).ItemDescription;
                    if (itemDescription.Length > 30)
                        itemDescription = itemDescription.Substring(0, 30) + "...";
                    else if (this.LineItems.Count() > 1)
