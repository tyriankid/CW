namespace Hidistro.UI.ControlPanel.Utility
{
    ...
    public class AdminPage : Page
    {
        ...
        protected virtual void CloseWindowAndRedirect()
        {
            string str = "var win = art.dialog.open.origin; win.location.href=win.location.href;";
            if (!this.Page.ClientScript.IsClientScriptBlockRegistered("ServerMessageScript"))
            {
        ...
        protected virtual void RegisterFrameScript()
        {
            ...
            //update by hj:更新后台菜单的显示机制,将静态页重做成动态页读取权限处理
            string script = string.Format("<script>if(window.parent.frames.length == 0) window.location.href=\"{0}\";</script>", Globals.ApplicationPath + "/admin/index.aspx");
        ...
        protected string GetPage(NameValueCollection queryStrings)
        {
            return this.GenericReloadUrl(queryStrings);
        }
