...
namespace Hidistro.UI.SaleSystem.CodeBehind
{
    ...
    public class VIndex : VWeiXinOAuthTemplatedWebControl
    {
        ...
        protected override void AttachChildControls()
        {
            ...
            int ireferralId = 0;
            if (currentMember != null && currentMember.DistributorUserId > 0)
            {
                ireferralId = currentMember.DistributorUserId;
            }
            else
            {
                string cookieReferralId = GetCurrReferralCookie();
                if (!string.IsNullOrEmpty(cookieReferralId))
                {
                    int.TryParse(cookieReferralId, out ireferralId);
                    WxLogger("Cookie中存储的用户门店ID值为" + ireferralId);
                }
            }
            userIdDistributors = DistributorsBrower.GetUserIdDistributors(ireferralId);
            ...
            */
            ...
            this.litItemParams.Text = string.Concat(new object[] { this.litItemParams.Text, str4, "|好店推荐之", str5, "商城|一个购物的好去处|", strUrl });
        ...
        private void distributorVisitCont()
        {
            ...
            if (currentMember != null && currentMember.OpenId != null)
            {
                ...
                DistributorsBrower.UpdateDistributorVisitCount(currentMember.UserId, currentMember.DistributorUserId);
            }
        }
