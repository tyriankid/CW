namespace Hidistro.UI.SaleSystem.CodeBehind
{
    ...
    public abstract class PaymentTemplatedWebControl : VshopTemplatedWebControl
    {
        ...
        private void UserPayOrder()
        {
            ...
            else if (this.Order.CheckAction(OrderActions.BUYER_PAY))
            {
                /*
                //付款成功修改订单状态
                ...
                {
                    //得到门店对应的账号值，该值将决定订单归属那个门店
                    store = StoreInfoHelper.GetStoreInfoByUserId(this.Order.ReferralUserId).accountALLHere;
                }
                string strmessage = string.Empty;
                int iresult = CwAPI.SendOrderToGankKou(this.Order, store, out strmessage);
                if (iresult == 0)
                    this.Order.submitgk = 1;//已成功推送数据到港口系统
                else
                    this.Order.submitgk = 0;//推送数据失败
                //记录操作日志
                WxLogger(strmessage);*/
        ...
        {
            string logFile = @"D:\Project\WebSite\CWWSC\gk_.txt";
