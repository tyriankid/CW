namespace Hidistro.UI.SaleSystem.CodeBehind
{
    ...
    public class VStatistics : VWeiXinOAuthTemplatedWebControl
    {
        ...
        protected override void AttachChildControls()
        {
            ...
            {
                int dzUserId = Globals.GetCurrentDistributorId();
                if (dzUserId > 0)
                {
                    currentMember = MemberProcessor.GetMember(dzUserId);//获取店长的实体信息对象
                    currentDistributor = DistributorsBrower.GetUserIdDistributors(dzUserId);//获取门店的实体信息对象
                }
            }
