namespace Hidistro.SqlDal.VShop
{
    ...
    public class ActivitiesDao
    {
        ...
        public ActivitiesInfo GetActivitiesInfoEx(int ActivitiesId)
        {
            ActivitiesInfo info = null;
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM Hishop_Activities WHERE ActivitiesId=@ActivitiesId");
            this.database.AddInParameter(sqlStringCommand, "ActivitiesId", DbType.Int32, ActivitiesId);
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                if (reader.Read())
        ...
        public DbQueryResult GetActivitiesList(ActivitiesQuery query)
        {
            ...
            return DataHelper.PagingByRownumber(query.PageIndex, query.PageSize, query.SortBy, query.SortOrder, query.IsCount, "Hishop_Activities ", "ActivitiesId", (builder.Length > 0) ? builder.ToString() : null, "*, (SELECT ProductName FROM Hishop_Products WHERE ProductId = Hishop_Activities.ActivitiesType) AS ProductName");
        }
        ...
        /// <returns></returns>
        public DataTable SelectActivitiesByProductId(int productid)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append(string.Format(@"select * from Hishop_Activities where ActivitiesType={0}", productid));
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(builder.ToString());
            return this.database.ExecuteDataSet(sqlStringCommand).Tables[0];
        }
    }
