namespace Hidistro.SqlDal.VShop
{
    ...
    public class PrizeRecordDao
    {
        ...
        public PrizeRecordInfo GetUserPrizeRecordById(int recordid)
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select top 1 * from Vshop_PrizeRecord where RecordId=@RecordId order by RecordId desc");
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(builder.ToString());
            this.database.AddInParameter(sqlStringCommand, "RecordId", DbType.Int32, recordid);
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                return ReaderConvert.ReaderToModel<PrizeRecordInfo>(reader);
            }
        ...
        public bool UpdatePrizeRecordById(PrizeRecordInfo model)
        {
            string query = "UPDATE Vshop_PrizeRecord SET  RealName=@RealName, CellPhone=@CellPhone WHERE RecordId=@RecordId";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
