namespace Hidistro.SqlDal.VShop
{
    ...
    public class LotteryActivityDao
    {
        ...
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select ActivityId,ActivityName,ActivityType,StartTime,EndTime,ActivityDesc,ActivityPic,ActivityKey,PrizeSetting,MaxNum,GradeIds from Vshop_LotteryActivity ");
        ...
        public DbQueryResult GetWKMList(WhoKnowMeQuery page)
        {
            StringBuilder builder = new StringBuilder();
            if (page.StartDate.HasValue)
            {
                builder.AppendFormat("StartDate>={0}", (DateTime) page.StartDate);
            }
            if (page.EndDate.HasValue)
            {
                builder.AppendFormat("EndDate<={0}", (DateTime)page.EndDate);
            }
            return DataHelper.PagingByRownumber(page.PageIndex, page.PageSize, page.SortBy, page.SortOrder, page.IsCount, "WKM_main", "StartTime", builder.ToString(), "*");
        }
        ...
        public List<PrizeRecordInfo> GetPrizeList(PrizeQuery page)
        {
            ...
            builder.Append("UserName=(select UserName from aspnet_Members c where  c.UserId=b.UserId),Address=(select Address from aspnet_Members c where  c.UserId=b.UserId),");
        ...
        public DataTable SelectLotteryActivity_Valid(int ActivityType, int UserID)
        {
            string selectSql = @"Select * From Vshop_LotteryActivity Where StartTime<=GETDATE() And EndTime>=GETDATE()
                                ";//And ActivityID NOT IN(Select ActivityID From Vshop_PrizeRecord Where UserID={0})
            selectSql = string.Format(selectSql, UserID);
            if (ActivityType > 0) selectSql += string.Format(" And ActivityType={0}", ActivityType);
            selectSql += " Order By StartTime";
            return this.database.ExecuteDataSet(CommandType.Text, selectSql).Tables[0];
        }

        public int SelectNumCount(int ActivityID, int currNumCount)
        {
            bool isChange = true;
            int result=currNumCount;
            string selectSql = string.Format("Select GradeIds From Vshop_LotteryActivity Where ActivityId={0}", ActivityID);
            object objGradeIds = this.database.ExecuteScalar(CommandType.Text, selectSql);
            if (objGradeIds!=null && !string.IsNullOrEmpty(objGradeIds.ToString())) {
                if (int.Parse(objGradeIds.ToString()) > currNumCount)
                {
                    result = int.Parse(objGradeIds.ToString());
                    isChange = false;
                }
            }
            if (isChange) {
                string updateSql = string.Format("Update Vshop_LotteryActivity Set GradeIds='{0}' Where ActivityId={1}", result, ActivityID);
                this.database.ExecuteNonQuery(CommandType.Text, updateSql);
            }
            return result;
        }
    }
