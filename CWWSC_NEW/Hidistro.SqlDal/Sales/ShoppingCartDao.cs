    ...
    public class ShoppingCartDao
    {
        ...
        public ShoppingCartItemInfo GetCartItemInfo(MemberInfo member, string skuId, int quantity)
        {
            ...
            using (IDataReader reader = this.database.ExecuteReader(storedProcCommand))
            {
                ...
                /******2017-8-4 yk针对线下商品的配送区域过滤********/
                if (reader["RegionId"]!=DBNull.Value)
                {
                    info.RegionId = reader["RegionId"].ToString();
                }
               /*************************************/
                ...
                //将来源及供应商ID存入购物车明细项中
                if (DBNull.Value != reader["ProductSource"])
                {
                    info.ProductSource = Convert.ToInt32(reader["ProductSource"]);
                }
                if (DBNull.Value != reader["SupplierId"])
                {
                    info.SupplierId = Convert.ToInt32(reader["SupplierId"]);
                }
                //商品编码
                if (DBNull.Value != reader["ProductCode"])
                {
                    info.ProductCode = reader["ProductCode"].ToString();
                }
                //分公司ID，线下订单使用
                if (DBNull.Value != reader["FilialeId"])
                {
                    info.FilialeId = Convert.ToInt32(reader["FilialeId"]);
                }
                //内购价
                if (DBNull.Value != reader["NeigouPrice"])
                {
                    info.NeigouPrice = (decimal)reader["NeigouPrice"];
                }
