namespace Hidistro.SqlDal
{
    ...
    public class RefundDao
    {
        ...
        public DbQueryResult GetReturnOrderAll(ReturnsApplyQuery returnsapplyquery)
        {
            ...
            if (returnsapplyquery.OrderSource > 0)
            {
                strOrder += string.Format("OrderSource = {0}", returnsapplyquery.OrderSource);
                if (returnsapplyquery.SupplierId > 0)
                {
                    strOrder += string.Format(" AND SupplierId = {0}", returnsapplyquery.SupplierId);
                }
                if (returnsapplyquery.ReferralUserId > 0)
                {
                    strOrder += string.Format(" AND ReferralUserId = {0}", returnsapplyquery.ReferralUserId);
                }
                if (returnsapplyquery.ServiceUserId > 0)
                {
                    strOrder += string.Format(" AND serviceUserId = {0}", returnsapplyquery.ServiceUserId);
                }
                if (returnsapplyquery.FilialeId > 0)
                {
                    strOrder += string.Format(" AND FilialeId = {0}", returnsapplyquery.FilialeId);
                }
            }
            if (!string.IsNullOrEmpty(strOrder))
                builder.AppendFormat(" AND OrderId in (select OrderId from dbo.Hishop_Orders where {0})", strOrder);
        ...
        public DataTable GetOrderReturnsByOrderid(string orderid)
        {
            string where = string.Empty;
            if (!string.IsNullOrEmpty(orderid)) where = " Where OrderId=' " + orderid + "' order by HandleTime desc";
            string selectSql = string.Format("Select top 1 * From Hishop_OrderReturns {0}", where);
            DataTable dtData = this.database.ExecuteDataSet(CommandType.Text, selectSql).Tables[0];
            return dtData;
        }
