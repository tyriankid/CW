namespace Hidistro.SqlDal.Orders
{
    ...
    public class OrderDao
    {
        ...
        public bool CreatOrder(OrderInfo orderInfo, DbTransaction dbTran)
        {
            ...
            this.database.AddInParameter(storedProcCommand, "OrderSource", DbType.Int32, orderInfo.OrderSource);
            this.database.AddInParameter(storedProcCommand, "SupplierId", DbType.Int32, orderInfo.SupplierId);
            this.database.AddInParameter(storedProcCommand, "receiptId", DbType.Int32, orderInfo.receiptId);
            this.database.AddInParameter(storedProcCommand, "buyType", DbType.Int32, orderInfo.buyType);
            this.database.AddInParameter(storedProcCommand, "serviceCode", DbType.String, orderInfo.serviceCode);
            this.database.AddInParameter(storedProcCommand, "serviceUserId", DbType.Int32, orderInfo.serviceUserId);
            this.database.AddInParameter(storedProcCommand, "FilialeId", DbType.Int32, orderInfo.FilialeId);
        ...
        public DataSet GetDistributorOrder(OrderQuery query)
        {
            ...
            {
                str += string.Format(" AND OrderDate >= '{0}' AND OrderDate < '{1}'", query.StartDate, query.EndDate);
            }
            if (query.OrderSource > 0)
            {
                str += string.Format(" AND OrderSource = {0}", query.OrderSource);
            }
        ...
        /// <param name="query"></param>
        /// <returns></returns>
        public DataSet GetDistributorOrderService(OrderQuery query)
        {
            string str = string.Empty;
            if (query.Status == OrderStatus.Finished)
            {
            ...
            //    str += string.Format(" AND OrderSource = {0}", query.OrderSource);
            //}
            string str2 = "SELECT Amount,RedPagerId,OrderId, OrderDate, OrderStatus,PaymentTypeId, OrderTotal,Gateway,FirstCommission,SecondCommission,ThirdCommission,ShippingModeId,ModeName,UserName,serviceCode,serviceUserId,serviceSalesId FROM Hishop_Orders o WHERE serviceUserId = @UserId";
            str2 = (str2 + str + " ORDER BY OrderDate DESC") + " SELECT OrderId,SkuId, ThumbnailsUrl=p.ThumbnailUrl180, ItemDescription, SKUContent, SKU, OI.ProductId,Quantity,ItemListPrice,ItemAdjustedCommssion,OrderItemsStatus,ItemsCommission FROM Hishop_OrderItems OI Left Join Hishop_Products P on oi.ProductId=p.ProductId WHERE OrderId IN (SELECT OrderId FROM Hishop_Orders WHERE serviceUserId = @UserId" + str + ");";
            str2 += "select * from Hishop_OrderGifts G WHERE OrderId IN (SELECT OrderId FROM Hishop_Orders WHERE serviceUserId =@UserId " + str + ")";
        ...
        /// <param name="query"></param>
        /// <returns></returns>
        public int GetDistributorOrderServiceCount(OrderQuery query)
        {
            string str = string.Empty;
            switch (query.Status)
            {
            ...
            string str3 = "SELECT COUNT(*)  FROM Hishop_Orders o WHERE serviceUserId = @serviceUserId";
            str3 = str3 + str;
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str3);
            sqlStringCommand.CommandType = CommandType.Text;
            this.database.AddInParameter(sqlStringCommand, "serviceUserId", DbType.Int32, query.UserId);
            return (int)this.database.ExecuteScalar(sqlStringCommand);
        }
        ...
        /// <param name="query"></param>
        /// <returns></returns>
        public int GetDistributorOrderAndServiceCount(OrderQuery query)
        {
            string str = string.Empty;
            switch (query.Status)
            {
            ...
            string str3 = "SELECT COUNT(*)  FROM Hishop_Orders o WHERE (ReferralUserId = @ReferralUserId OR serviceUserId = @serviceUserId)";
            str3 = str3 + str;
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str3);
            sqlStringCommand.CommandType = CommandType.Text;
            this.database.AddInParameter(sqlStringCommand, "ReferralUserId", DbType.Int32, query.UserId);
        ...
        {
            OrderInfo info = null;
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM Hishop_Orders Where Id = @Id;  SELECT *,p.CategoryId FROM Hishop_OrderItems OI inner join Hishop_Products P on oi.ProductId=p.ProductId Where OrderId in (select OrderId from Hishop_Orders where Id = @Id); Select * from Hishop_OrderGifts where OrderID in (select OrderId from Hishop_Orders where Id = @Id) ");
        ...
        public DbQueryResult GetOrders(OrderQuery query)
        {
            ...
            if (query.referralUserId.HasValue && query.referralUserId.Value > 0)
            {
                builder.AppendFormat(" AND ReferralUserId = '{0}'", query.referralUserId.Value);
            }
            if (!string.IsNullOrEmpty(query.FilialeIds))
            {
                if (query.FilialeIds != "null")
                    builder.AppendFormat(" AND ReferralUserId in ({0})", query.FilialeIds);
                else
                    builder.Append(" AND ReferralUserId = -1");
            }
            ...
            {
                builder.AppendFormat(" AND  accountALLHere  like '%{0}%' ", DataHelper.CleanSearchString(query.AllHereCode));
            }
            ...
            if (query.OrderSource > 0)
            {
                builder.AppendFormat(" AND OrderSource = '{0}'", query.OrderSource);
            }
            if (query.SupplierId > 0)
            {
                builder.AppendFormat(" AND SupplierId = '{0}'", query.SupplierId);
            }
            if (query.FilialeId.HasValue && query.FilialeId.Value > 0)
            {
                builder.AppendFormat(" AND FilialeId = '{0}'", query.FilialeId.Value);
            }
            if (!string.IsNullOrEmpty(query.fgsName))
            {
                builder.AppendFormat(" AND fgsName like '%{0}%'", query.fgsName);
            }
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(string.Format("SELECT * FROM Hishop_Orders WHERE (OrderStatus = 2 OR (OrderStatus = 1 AND Gateway = 'hishop.plugins.payment.podrequest')) AND submitgk = 1 AND OrderId IN ({0}) order by OrderDate desc", orderIds));
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                return DataHelper.ConverDataReaderToDataTable(reader);
            }
        ...
        public DataSet GetUserOrder(int userId, OrderQuery query)
        {
            ...
            {
                str += " AND OrderStatus = 1 AND Gateway <> 'hishop.plugins.payment.podrequest'";
            }
            else if (query.Status == OrderStatus.SellerAlreadySent)
            {
                str += " AND (OrderStatus = 2 OR OrderStatus = 3 OR (OrderStatus = 1 AND Gateway = 'hishop.plugins.payment.podrequest'))";
            }
            if (query.OrderSource > 0)
            {
                str += string.Format(" AND OrderSource = {0}", query.OrderSource);
            ...
            //else
            //    str += " AND (OrderSource = 1 or OrderSource = 2)";
            string str2 = "SELECT OrderId, OrderDate, OrderSource, OrderStatus,PaymentTypeId, OrderTotal, Gateway,(SELECT count(0) FROM vshop_OrderRedPager WHERE OrderId = o.OrderId and ExpiryDays<getdate() and AlreadyGetTimes<MaxGetTimes) as HasRedPage,(SELECT SUM(Quantity) FROM Hishop_OrderItems WHERE OrderId = o.OrderId) as ProductSum,(select SUM(Quantity) from Hishop_OrderGifts G WHERE OrderId = o.OrderId)as GiftSum,(select SUM(costPoint)from Hishop_OrderGifts G WHERE OrderId = o.OrderId)as PointSum FROM Hishop_Orders o WHERE UserId = @UserId";
        ...
        /// <param name="query"></param>
        /// <returns></returns>
        public DataSet GetUserServiceOrder(Guid salesuserId, OrderQuery query)
        {
            string str = string.Empty;
            //if (query.Status == OrderStatus.WaitBuyerPay)
            //{
            ...
            {
                str += " AND (OrderStatus = 5 OR OrderStatus = 88)";
            }
            if (query.OrderSource > 0)
            {
                str += string.Format(" AND OrderSource = {0}", query.OrderSource);
            }
            string str2 = "SELECT OrderId, OrderDate, OrderSource, OrderStatus,PaymentTypeId, OrderTotal, Gateway,(SELECT count(0) FROM vshop_OrderRedPager WHERE OrderId = o.OrderId and ExpiryDays<getdate() and AlreadyGetTimes<MaxGetTimes) as HasRedPage,(SELECT SUM(Quantity) FROM Hishop_OrderItems WHERE OrderId = o.OrderId) as ProductSum,(select SUM(Quantity) from Hishop_OrderGifts G WHERE OrderId = o.OrderId)as GiftSum,(select SUM(costPoint)from Hishop_OrderGifts G WHERE OrderId = o.OrderId)as PointSum FROM Hishop_Orders o WHERE serviceSalesId = @ServiceSalesId";
            str2 = (str2 + str + " ORDER BY OrderDate DESC") + " SELECT OrderId, ThumbnailsUrl=p.ThumbnailUrl180, ItemDescription, SKUContent, SKU,OrderItemsStatus, OI.ProductId,Quantity,ItemAdjustedPrice FROM Hishop_OrderItems OI Left Join Hishop_Products P on oi.ProductId=p.ProductId WHERE OrderId IN (SELECT OrderId FROM Hishop_Orders WHERE serviceSalesId = @ServiceSalesId" + str + ");";
            str2 += "select * from Hishop_OrderGifts G WHERE OrderId IN (SELECT OrderId FROM Hishop_Orders WHERE serviceSalesId =@ServiceSalesId " + str + ")";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str2);
            this.database.AddInParameter(sqlStringCommand, "ServiceSalesId", DbType.Guid, salesuserId);
            DataSet set = this.database.ExecuteDataSet(sqlStringCommand);
            DataColumn parentColumn = set.Tables[0].Columns["OrderId"];
            DataColumn childColumn = set.Tables[1].Columns["OrderId"];
            DataRelation relation = new DataRelation("OrderItems", parentColumn, childColumn, false);
        ...
        {
            string str = string.Empty + " AND (OrderStatus = 2 OR OrderStatus = 3 OR OrderStatus = 88)";
            string str2 = "SELECT OrderId, OrderDate, OrderStatus,PaymentTypeId, OrderTotal, TakeGoodsDate, (SELECT SUM(Quantity) FROM  Hishop_OrderItems WHERE OrderId = o.OrderId) as ProductSum FROM Hishop_Orders o WHERE UserId = @UserId";
            str2 = (str2 + str + " ORDER BY OrderDate DESC") + " SELECT OrderId, ThumbnailsUrl=p.ThumbnailUrl180,Quantity, (SELECT TakeGoodsDate FROM  Hishop_Orders WHERE OrderId = OI.OrderId) as TakeGoodsDate,  ItemDescription,OrderItemsStatus, SKUContent, SKU, OI.ProductId,P.ProductSource,P.KukaiCode  FROM Hishop_OrderItems OI Left Join Hishop_Products P on  oi.ProductId=p.ProductId  WHERE (OrderItemsStatus=2 OR OrderItemsStatus=3 OR OrderItemsStatus=88) AND OrderId IN (SELECT OrderId FROM Hishop_Orders WHERE  UserId = @UserId" + str + ")";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(str2);
            this.database.AddInParameter(sqlStringCommand, "UserId", DbType.Int32, userId);
            DataSet set = this.database.ExecuteDataSet(sqlStringCommand);
            DataColumn parentColumn = set.Tables[0].Columns["OrderId"];
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(string.Format("UPDATE Hishop_Orders SET ExpressCompanyName=@ExpressCompanyName,ExpressCompanyAbb=@ExpressCompanyAbb WHERE (OrderStatus = 2 OR (OrderStatus = 1 AND Gateway='hishop.plugins.payment.podrequest')) AND submitgk = 1 AND OrderId IN ({0})", orderIds));
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(string.Format("UPDATE Hishop_Orders SET RealShippingModeId=@RealShippingModeId,RealModeName=@RealModeName WHERE (OrderStatus = 2 OR (OrderStatus = 1 AND Gateway='hishop.plugins.payment.podrequest')) AND submitgk = 1 AND OrderId IN ({0})", orderIds));
        ...
        /// <param name="submitgk"></param>
        public void UpdateSubmitgk(string orderId, int submitgk)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("Update Hishop_Orders Set submitgk=@submitgk Where OrderId =@OrderId");
            this.database.AddInParameter(sqlStringCommand, "submitgk", DbType.Int32, submitgk);
            this.database.AddInParameter(sqlStringCommand, "OrderId", DbType.String, orderId);
            this.database.ExecuteNonQuery(sqlStringCommand);
        }
        ...
        /// <param name="orderId"></param>
        /// <param name="submitgk"></param>
        public void UpdateSubmitkk(string orderId, int submitgk)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("Update Hishop_Orders Set submitkk=@submitkk Where OrderId =@OrderId");
            this.database.AddInParameter(sqlStringCommand, "submitkk", DbType.Int32, submitgk);
            this.database.AddInParameter(sqlStringCommand, "OrderId", DbType.String, orderId);
            this.database.ExecuteNonQuery(sqlStringCommand);
        }
        ...
        public void UpdateSerialNum(string orderId, string serialnum)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("Update Hishop_Orders Set serialNumber=@serialNumber Where OrderId =@OrderId");
            this.database.AddInParameter(sqlStringCommand, "serialNumber", DbType.String, serialnum);
            this.database.AddInParameter(sqlStringCommand, "OrderId", DbType.String, orderId);
            this.database.ExecuteNonQuery(sqlStringCommand);
        }
        ...
        public bool UpdateOrder(OrderInfo order, DbTransaction dbTran = null)
        {
            ...
            this.database.AddInParameter(sqlStringCommand, "submitgk", DbType.Int32, order.submitgk);//是否同步港口/金力
            this.database.AddInParameter(sqlStringCommand, "submitkk", DbType.Int32, order.submitkk);//是否同步酷开
            this.database.AddInParameter(sqlStringCommand, "TakeGoodsDate", DbType.DateTime, order.TakeGoodsDate);
        ...
        /// <param name="orderid"></param>
        /// <returns></returns>
        public DataTable GetCommissionDetails(string orderid,string fgsname = "", string storename = "",string accountallhere="", string starttime = "", string endtime = "", string storeids = "")
        {
...
//             inner join aspnet_Distributors AD on ho.UserId=ad.UserId
//             left join dbo.CW_StoreInfo as csi on AD.StoreId = csi.id 
//                left join dbo.CW_Filiale as cf on csi.fgsid = cf.id where State = 1 ");
            string sql = string.Format(@"
            select oi.OrderId 订单,cf.fgsName 分公司,csi.accountALLHere ALLHere帐号,csi.storename 门店,佣金=RTRIM(ItemsCommission),
            productname 产品名称,ItemAdjustedPrice 订单金额,Quantity 数量,CONVERT(char(20),CI.TradeTime,20) as 时间
            from Hishop_Commissions as CI 
            left join Hishop_Orders as HO on CI.OrderId=HO.OrderId 
            left join Hishop_OrderItems as OI on OI.OrderId = HO.OrderId 
            inner join Hishop_Products as P on OI.ProductId = P.ProductId 
            left join aspnet_Distributors as AD on ho.ReferralUserId = AD.UserId 
            left join dbo.CW_StoreInfo as csi on AD.StoreId = csi.id 
            left join dbo.CW_Filiale as cf on csi.fgsid = cf.id where State = 1 ");
            if (!string.IsNullOrEmpty(orderid))
            {
                sql += string.Format(" and CI.OrderId='{0}'  ", orderid);
            }
            if (!string.IsNullOrEmpty(storeids))
            {
                sql += string.Format(" and AD.UserId in ({0})  ", storeids);
            }
            if (!string.IsNullOrEmpty(fgsname))
            {
                sql += string.Format(" and cf.fgsName like '%{0}%'", DataHelper.CleanSearchString(fgsname));
            }
            if (!string.IsNullOrEmpty(storename))
            {
                sql += string.Format(" and csi.storeName like '%{0}%'", DataHelper.CleanSearchString(storename));
            }
            if (!string.IsNullOrEmpty(accountallhere))
            {
                sql += string.Format(" and csi.accountallhere like '%{0}%'", DataHelper.CleanSearchString(accountallhere));
            }
            if (starttime != "")
            {
                sql += string.Format(" AND datediff(dd,'{0}',CONVERT(char(20),CI.TradeTime,20))>=0", starttime);
            }
            if (endtime != "")
            {
                sql += string.Format(" AND datediff(dd,'{0}',CONVERT(char(20),CI.TradeTime,20))<=0", endtime);
            }
        ...
        public DataTable GetOrder(OrderQuery query)
        {
            ...
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("select *,(DiscountAmount+ CouponValue) as WxhdAmount FROM vw_Hishop_Order WHERE  " + builder.ToString());
        ...
        public DataTable GetOrderSanZuo(OrderQuery query)
        {
            ...
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("select * FROM  vw_Hishop_Order as Ho left Join Hishop_OrderItems as HOI on ho.OrderId=hoi.OrderId  Join Hishop_Products as HP on hoi.ProductId=hp.ProductId  WHERE  " + builder.ToString());
            return this.database.ExecuteDataSet(sqlStringCommand).Tables[0];
        ...
        /// 根据时间段获取orderId列表
        /// </summary>
        public System.Collections.Generic.IList<string> GetTodayOrders(string startDate, string endDate, int senderId = 0, string storeName = "")
        {
        ...
        /// 根据时间段获取商品列表
        /// </summary>
        public DataTable GetTodayProducts(string startDate, string endDate, int senderId = 0, string storeName="")
        {
        ...
        //根据条件查询订单
        public DataTable GetOrderByWhere(string where)
        {
            string sql = string.Format("SELECT * FROM vw_Hishop_Order ");
        ...
        /// <returns></returns>
        public DataTable GetOrderItemByUserIdAndProId(int userId, int productId)
        {
            string sql = string.Format("SELECT * FROM dbo.Hishop_Orders AS o LEFT JOIN dbo.Hishop_OrderItems AS oi on o.OrderId = oi.OrderId where o.OrderStatus <> 4 and o.UserId = {0} and oi.ProductId = {1} and o.buyType = 4 ", userId, productId);
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(sql);
            return this.database.ExecuteDataSet(sqlStringCommand).Tables[0];
        }
