    ...
    public class LineItemDao
    {
        ...
        public bool AddOrderLineItems(string orderId, ICollection lineItems, DbTransaction dbTran)
        {
            ...
            foreach (LineItemInfo info in lineItems)
            {
                ...
                builder.Append("INSERT INTO Hishop_OrderItems(OrderId, SkuId, ProductId, SKU, Quantity,GiveQuantity, ShipmentQuantity, CostPrice").Append(",ItemListPrice, ItemAdjustedPrice, ItemDescription, ThumbnailsUrl, Weight, SKUContent, PromotionId, PromotionName,OrderItemsStatus,ItemsCommission,SecondItemsCommission,ThirdItemsCommission) VALUES(@OrderId").Append(",@SkuId").Append(str).Append(",@ProductId").Append(str).Append(",@SKU").Append(str).Append(",@Quantity").Append(str).Append(",@GiveQuantity").Append(str).Append(",@ShipmentQuantity").Append(str).Append(",@CostPrice").Append(str).Append(",@ItemListPrice").Append(str).Append(",@ItemAdjustedPrice").Append(str).Append(",@ItemDescription").Append(str).Append(",@ThumbnailsUrl").Append(str).Append(",@Weight").Append(str).Append(",@SKUContent").Append(str).Append(",@PromotionId").Append(str).Append(",@PromotionName").Append(str).Append(",@OrderItemsStatus").Append(str).Append(",@ItemsCommission").Append(str).Append(",@SecondItemsCommission").Append(str).Append(",@ThirdItemsCommission").Append(str).Append(");");
                ...
                this.database.AddInParameter(sqlStringCommand, "GiveQuantity" + str, DbType.Int32, info.GiveQuantity);
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT ISNULL(SUM(quantity),0) AS Quanity FROM Hishop_OrderItems WHERE ProductID=@ProductID AND  OrderId in(SELECT orderid FROM Hishop_Orders WHERE UserID=@UserID AND orderstatus!=@OrderClosed AND orderstatus!=@OrderRefuned AND ISNULL(CountDownBuyId, 0) > 0)");
            this.database.AddInParameter(sqlStringCommand, "ProductID", DbType.Int32, productid);
            this.database.AddInParameter(sqlStringCommand, "UserID", DbType.Int32, Globals.GetCurrentMemberUserId());
            this.database.AddInParameter(sqlStringCommand, "OrderClosed", DbType.Int32, 4);
            this.database.AddInParameter(sqlStringCommand, "OrderRefuned", DbType.Int32, 9);
