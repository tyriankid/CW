namespace Hidistro.SqlDal.Orders
{
    ...
    public class ReturnDao
    {
        ...
        public  string GetReturnsIdByOrderId(string orderid)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("select * from Hishop_OrderReturns where OrderId=@OrderId");
            this.database.AddInParameter(sqlStringCommand, "OrderId", DbType.String, orderid);
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    return dt.Rows[0]["ReturnsId"].ToString();
                }
                else
                {
                    return null;
