...
namespace Hidistro.SqlDal.Commodities
{
    ...
    public class WxPoiDao
    {
        ...
        public DataTable GetPoiListInfo()
        {
            ...
            DbCommand selectCommand = this.database.GetSqlStringCommand("select AM.userid as sender,AM.UserName,WP.* from wx_poiInfo WP left join aspnet_Managers AM on wp.poi_id = AM.poi_id");
        ...
        {
            DbCommand selectCommand = this.database.GetSqlStringCommand(string.Format("select distributorId,RegionName from YiHui_DistributorRegion where distributorId in ({0})", storeIds));
            using (IDataReader reader = this.database.ExecuteReader(selectCommand))
            {
                return DataHelper.ConverDataReaderToDataTable(reader);
            }
