namespace Hidistro.SqlDal.Commodities
{
    ...
    public class ProductDao
    {
        ...
        /// <summary>
        /// 获取所有商品基本信息  2017-7-27 yk
        /// </summary>
        /// <returns></returns>
        public DataTable GetStoreProductBaseInfo(string where = "")
        {

            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("  select (SELECT dbo.Fn_GetProductSKU(ProductId)) AS SKU,ProductId,ImageUrl1,ProductName FROM Hishop_Products  " + where + "");
        ...
        /// <returns></returns>
        public int SelectFilialeProductNum(string ProductCode, int  FilialeId)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(string.Format("select count(*) from Hishop_Products PD left join CW_Filiale f on pd.FilialeId =f.id where id={0} and ProductCode='{1}'",FilialeId,ProductCode));
            return (int)this.database.ExecuteScalar(sqlStringCommand);
        }
        /// <summary>
        ///根据商品码查询分公司对该线下商品的上架情况
        /// </summary>
        /// <param name="where"></param>
        /// <returns></returns>
        public DataTable GetFilialeProductDetail(string productCode)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(string.Format("select  ProductId,fgsName,ProductName,(SELECT CostPrice FROM Hishop_SKUs WHERE SkuId = PD.SkuId) AS  CostPrice,ProductCode,Stock,SalePrice,MarketPrice,SaleStatus,RegionName,ThumbnailUrl40  from vw_Hishop_BrowseProductList PD  left join CW_Filiale CF on PD.FilialeId=CF.id where  PD.FilialeId>0 and productCode='{0}'", productCode));
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                return DataHelper.ConverDataReaderToDataTable(reader);
            }
        ...
        public int AddProduct(ProductInfo product, DbTransaction dbTran)
        {
            ...
            this.database.AddInParameter(storedProcCommand, "ProductSource", DbType.Int32, product.ProductSource);
            this.database.AddInParameter(storedProcCommand, "SupplierId", DbType.Int32, product.SupplierId);
            this.database.AddInParameter(storedProcCommand, "specification", DbType.String, product.specification);//*
            this.database.AddInParameter(storedProcCommand, "RestrictNeigouNum", DbType.Int32, product.RestrictNeigouNum);//限购数量
            this.database.AddInParameter(storedProcCommand, "KukaiCode", DbType.String, product.KukaiCode);//酷开商品编码
            this.database.AddInParameter(storedProcCommand, "ClassId", DbType.Int32, product.ClassId);//服务商品所属服务品类2017-07-21
            this.database.AddInParameter(storedProcCommand, "FilialeId", DbType.Int32, product.FilialeId);//分公司Id（商品来源为4线下，此字段才生效） 2017-8-1 yk
            this.database.AddInParameter(storedProcCommand, "RegionId", DbType.Int32, product.RegionId);//分公司销售区域ID
            this.database.AddInParameter(storedProcCommand, "RegionName", DbType.String, product.RegionName);//分公司销售区域NAME
            /**start设置佣金***/
            this.database.AddInParameter(storedProcCommand, "SaleCommisionType", DbType.Int32, product.SaleCommisionType);
            this.database.AddInParameter(storedProcCommand, "SaleCommisionRatio", DbType.Decimal, product.SaleCommisionRatio);
            this.database.AddInParameter(storedProcCommand, "SaleCommisionMoney", DbType.Currency, product.SaleCommisionMoney);
            this.database.AddInParameter(storedProcCommand, "NeigouCommisionType", DbType.Int32, product.NeigouCommisionType);
            this.database.AddInParameter(storedProcCommand, "NeigouCommisionRatio", DbType.Decimal, product.NeigouCommisionRatio);
            this.database.AddInParameter(storedProcCommand, "NeigouCommisionMoney", DbType.Currency, product.NeigouCommisionMoney);
            this.database.AddInParameter(storedProcCommand, "wholesaleCommisionType", DbType.Int32, product.wholesaleCommisionType);
            this.database.AddInParameter(storedProcCommand, "wholesaleCommisionRatio", DbType.Decimal, product.wholesaleCommisionRatio);
            this.database.AddInParameter(storedProcCommand, "wholesaleCommisionMoney", DbType.Currency, product.wholesaleCommisionMoney);
            /**end设置佣金***/
        ...
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("INSERT INTO Hishop_SKUs(SkuId, ProductId, SKU, Weight, Stock, CostPrice, SalePrice, NeigouPrice) VALUES(@SkuId, @ProductId, @SKU, @Weight, @Stock, @CostPrice, @SalePrice, @NeigouPrice)");
            ...
            this.database.AddInParameter(sqlStringCommand, "NeigouPrice", DbType.Currency);
            ...
            try
            {
                ...
                foreach (SKUItem item in skus.Values)
                {
                    ...
                    this.database.SetParameterValue(sqlStringCommand, "NeigouPrice", item.NeigouPrice);
        ...
        public ProductQuery GetHishop_ProductsId(int ProductId)
            {
                DbCommand sqlStringCommand = this.database.GetSqlStringCommand(string.Format("select * from Hishop_Products where ProductId='{0}'", ProductId));
                using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
                {      
        ...
        public DbQueryResult GetProducts(ProductQuery query)
        {
            ...
            {
                if (query.SourceId.Value > 0)
                    builder.AppendFormat(" AND SupplierId = {0}", query.SourceId.Value);
                else
                    builder.AppendFormat(" AND (SupplierId = 0 or SupplierId is null)");
            }
            ...
            //供应商相关商品筛选
            if (query.ProductSource >= 0)
            {
                builder.AppendFormat(" AND ProductSource ='{0}'", query.ProductSource);
            }
            if (query.SupplierId > 0)
            {
                builder.AppendFormat(" AND SupplierId ='{0}'", query.SupplierId);
            }
            if (query.ClassId.HasValue && query.ClassId > 0)
            {
                builder.AppendFormat(" AND  ClassId='{0}'", query.ClassId.Value);
            }
            string selectFields = "ProductId, ProductCode,KukaiCode,IsMakeTaobao,ProductName, ThumbnailUrl40, MarketPrice, SalePrice, (SELECT CostPrice FROM Hishop_SKUs WHERE SkuId = p.SkuId) AS  CostPrice,FilialeId,Stock, DisplaySequence,SaleStatus";
            return DataHelper.PagingByRownumber(query.PageIndex, query.PageSize, query.SortBy, query.SortOrder, query.IsCount, "vw_Hishop_BrowseProductList p", "ProductId", builder.ToString(), selectFields);
        }
        ...
        public DbQueryResult GetProductsAndCode(ProductQuery query)
        {
            ...
            if (!string.IsNullOrEmpty(query.Keywords))
            {
                query.Keywords = DataHelper.CleanSearchString(query.Keywords);
                string[] strArray = Regex.Split(query.Keywords.Trim(), @"\s+");
                builder.AppendFormat(" AND ProductName LIKE '%{0}%' OR CommoditySource  LIKE '%{0}%' OR CommodityCode  LIKE '%{0}%'", DataHelper.CleanSearchString(strArray[0]));
                for (int i = 1; (i < strArray.Length) && (i <= 4); i++)
                {
                    builder.AppendFormat("AND ProductName LIKE '%{0}%' OR CommoditySource  LIKE '%{0}%' OR CommodityCode  LIKE '%{0}%'", DataHelper.CleanSearchString(strArray[i]));
                }
            }
            if (query.IsMakeTaobao.HasValue && (query.IsMakeTaobao.Value >= 0))
            {
            ...
            if (query.ClassId.HasValue && query.ClassId.Value > 0)
            {
                builder.AppendFormat(" AND ClassId={0}", query.ClassId.Value);
            }
            string selectFields = "*,stuff((select ','+CommodityCode from hishop_ProductsList where CommodityID = vw_Hishop_BrowseProductList.ProductId for xml path('')),1,1,'') as CommodityCode ,stuff((select ','+CommoditySource from hishop_ProductsList where CommodityID = vw_Hishop_BrowseProductList.ProductId for xml path('')),1,1,'') as CommoditySource";
        ...
        public bool UpdateProduct(ProductInfo product, DbTransaction dbTran)
        {
            ...
            this.database.AddInParameter(storedProcCommand, "ProductSource", DbType.Int32, product.ProductSource);
            this.database.AddInParameter(storedProcCommand, "SupplierId", DbType.Int32, product.SupplierId);
            this.database.AddInParameter(storedProcCommand, "RestrictNeigouNum", DbType.Int32, product.RestrictNeigouNum);//内购数量
            this.database.AddInParameter(storedProcCommand, "KukaiCode", DbType.String, product.KukaiCode);//酷开商品编码
            this.database.AddInParameter(storedProcCommand, "ClassId", DbType.Int32, product.ClassId);//酷开商品编码
            this.database.AddInParameter(storedProcCommand, "FilialeId", DbType.Int32, product.FilialeId);//分公司Id（商品来源为4线下，此字段才生效）
            this.database.AddInParameter(storedProcCommand, "RegionId", DbType.Int32, product.RegionId);//分公司销售区域ID
            this.database.AddInParameter(storedProcCommand, "RegionName", DbType.String, product.RegionName);//分公司销售区域NAME
            /**start设置佣金***/
            this.database.AddInParameter(storedProcCommand, "SaleCommisionType", DbType.Int32, product.SaleCommisionType);
        ...
        /// <returns>数据集</returns>
        public DataTable GetProductSkuItem(int productid)
        {
            string strsql = string.Format(@"select sku.*,attr.AttributeName,attrvalue.ValueStr from dbo.Hishop_SKUs as sku left join Hishop_SKUItems as skuitem on sku.SkuId = skuitem.SkuId 
                left join dbo.Hishop_AttributeValues as attrvalue on attrvalue.ValueId = skuitem.ValueId 
                left join dbo.Hishop_Attributes as attr on attr.AttributeId = attrvalue.AttributeId 
