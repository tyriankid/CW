...
using System.Text.RegularExpressions;
    ...
    public class ProductBrowseDao
    {
        ...
        public ProductInfo GetProduct(MemberInfo member, int productId)
        {
            ...
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM Hishop_Products WHERE ProductId =@ProductId;SELECT SkuId, ProductId, SKU,Weight, Stock, CostPrice,NeigouPrice, " + builder.ToString() + " FROM Hishop_SKUs s WHERE ProductId = @ProductId");
        ...
        public DataTable GetProductsRange(MemberInfo member, int? topicId, int? categoryId, int? valueId, int distributorId, string keyWord, int pageNumber, int maxNum, out int toal, string sort, bool isAsc = false, ProductInfo.ProductRanage productRanage = ProductInfo.ProductRanage.NormalSelect, string swr = "", int rangeType = 0,string fgsId="")
        {
            int discount = 100;
            StringBuilder builder = new StringBuilder();
            builder.AppendFormat("ProductId,CategoryId,SkuId,Stock,ProductName,ProductCode,ShowSaleCounts AS SaleCounts,ShortDescription,", maxNum);
            builder.Append(" ThumbnailUrl60,ThumbnailUrl100,ThumbnailUrl160,ThumbnailUrl180,ThumbnailUrl220,ThumbnailUrl310,MarketPrice,VistiCounts,ProductSource,");
            ...
            /*****start2017-08-03 yk,门店过滤分公司商品的显示******/
            //获取当前门店的分公司ID
            if (!string.IsNullOrEmpty(fgsId))
            {
                builder2.AppendFormat(string.Format(" and  (ProductSource <> 4  or (ProductSource = 4 and FilialeId ={0}))", fgsId));
            }
            //若没有分公司ID 则为总店 屏蔽线下商品
            else
            {
                builder2.Append(" and  ProductSource <> 4 ");
            }
            ...
            {
                builder2.AppendFormat(" AND ProductId IN (SELECT ProductId FROM Hishop_ProductAttributes where ValueId = {0})", valueId.Value);
            }
        ...
        public DataTable GetProductsEx(MemberInfo member, int? topicId, int? categoryId, int distributorId, string keyWord, int pageNumber, int maxNum, out int toal, string sort, bool isAsc = false
            , ProductInfo.ProductRanage productRanage = ProductInfo.ProductRanage.NormalSelect, string fgsId="")
        {
            int discount = 100;
            StringBuilder builder = new StringBuilder();
            ...
            StringBuilder builder2 = new StringBuilder();
            builder2.Append(" SaleStatus=1");
            //---star  2017-8-3 yk  加入分公司线下商品过滤
            if (!string.IsNullOrEmpty(fgsId))
            {
                builder2.AppendFormat(" And ProductSource = 4 and FilialeId ={0}", fgsId);
            }
            else
            {
                builder2.AppendFormat(" And ProductSource <> 4", fgsId);
            }
            //---end
            if (topicId.HasValue)
            {
        ...
        public DataTable GetProducts(string where)
        {
            ...
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT ProductId,ProductName,ProductCode FROM Hishop_Products" + strWhere);
            using (IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
            {
                return DataHelper.ConverDataReaderToDataTable(reader);
            }
