namespace Hidistro.ControlPanel.Members
{
    ...
    public static class MemberHelper
    {
        ...
        public static string GetAgentDistributorsName(string keysearch)
        {
            string str = "";
            foreach (DataRow row in new DistributorsDao().GetAgentDistributorsName(keysearch).Rows)
            {
                string str2 = str;
                str = str2 + "{\"title\":\"" + Globals.HtmlEncode(row[0].ToString()) + "\",\"result\":\"" + row[1].ToString() + "\"},";
            }
        ...
        public static string GetDistributorsName(string keysearch)
        {
            string str = "";
            foreach (DataRow row in new DistributorsDao().GetDistributorsName(keysearch).Rows)
            {
                string str2 = str;
                str = str2 + "{\"title\":\"" + Globals.HtmlEncode(row[0].ToString()) + "\",\"result\":\"" + row[1].ToString() + "\"},";
            }
        ...
        public static DbQueryResult GetFullMembers(MemberQuery query)
        {
            return new MemberDao().GetFullMembers(query);
        }

        public static DbQueryResult GetMembersEx(MemberQuery query)
        {
            return new MemberDao().GetMembersEx(query);
        }
        ...
        public static DataTable GetMembersViscidity(MemberQuery query, IList<string> fields)
        {
            return new MemberDao().GetMembersViscidity(query, fields);
        }
        ...
        public static IList<MemberInfo> GetMemdersByOpenIds(string openids,int t)
        {
            return new MemberDao().GetMemdersByOpenIds(openids,t);
        }
