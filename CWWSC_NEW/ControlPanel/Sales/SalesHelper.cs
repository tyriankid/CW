namespace Hidistro.ControlPanel.Sales
{
    ...
    public sealed class SalesHelper
    {
        ...
        public static void GetNewlyOrdersCountAndPayCount(DateTime lastTime, out int ordersCount, out int payCount, out int RefundOrderCount, out int ReplacementOrderCount, out int ReturnsOrderCount, bool isSupplier, bool isStoreId, int clientUserId)
        {
            ordersCount = 0;
            payCount = 0;
            RefundOrderCount = 0;
            ...
            ReturnsOrderCount = 0;
            new SaleStatisticDao().GetNewlyOrdersCountAndPayCount(lastTime, out ordersCount, out payCount, out RefundOrderCount, out ReplacementOrderCount, out ReturnsOrderCount, isSupplier, isStoreId, clientUserId);
        }
        ...
        /// <returns></returns>
        public static IList<UserStatisticsForDate> GetDistributorsAdd(int? year, int? month, int? days)
        {
            return new DateStatisticDao().GetDistributorsAdd(year, month, days);
        }
        ...
        public static DataTable GetFilialeStatisticsPage(FilialeStatisticsQuery Filiale, string where1, string where2, string where3, string where4, out int count)
        {
            return new SaleStatisticDao().FilialeStatisticsInfo(Filiale, where1, where2, where3, where4, out count);
        }
        ...
        public static DataTable GetProductQuantity(string StartTime = "", string endTime = "",int managerId = 0)
        {
            return new SaleStatisticDao().GetProductQuantity(StartTime, endTime, managerId);
        }
