    ...
    public static class DataHelper
    {
        ...
        private static string BuildRownumberQuery(string sortBy, SortAction sortOrder, bool isCount, string table, string pk, string filter, string selectFields, int partitionSize)
        {
            ...
            if (isCount && (partitionSize == 0))
            {
                ...
                builder.AppendFormat(";SELECT COUNT({0}) FROM {1} {2}",  pk , table, str);//完善BUG修改，如果排序字段存在空值则出错。
            }
            return builder.ToString();
        }
        ...
        private static string BuildTopQuery(int pageIndex, int pageSize, string sortBy, SortAction sortOrder, bool isCount, string table, string pk, string filter, string selectFields)
        {
            ...
            string str = string.IsNullOrEmpty(sortBy) ? pk : (sortBy + " " + (sortOrder == SortAction.Asc ? " ASC " : " DESC ") + (sortBy.Equals(pk) ? "" : ("," + pk)));
            string str1 = pk;//主键
            string str2 = string.IsNullOrEmpty(filter) ? "" : ("WHERE " + filter);
            string str3 = string.IsNullOrEmpty(filter) ? "" : ("AND " + filter);
            StringBuilder builder = new StringBuilder();
            ...
            if (pageIndex == 1)
            {
                builder.AppendFormat("{0} ORDER BY {1} ", str2, str);
            }
            else
            {
                int num = (pageIndex - 1) * pageSize;
                builder.AppendFormat("WHERE {0} > (SELECT MAX({0}) FROM (SELECT TOP {1} {0} FROM {2} {3} ORDER BY {0} ASC) AS TMP) {4} ORDER BY {5} ", new object[] { str1, num, table, str2, str3, str });
                //if (sortOrder == SortAction.Asc)
                //{
                //    builder.AppendFormat("WHERE {0} > (SELECT MAX({0}) FROM (SELECT TOP {1} {0} FROM {2} {3} ORDER BY {0} ASC) AS TMP) {4} ORDER BY {5} ASC", new object[] { str1, num, table, str2, str3, str });
                //}
                //else
                //{
                //    builder.AppendFormat("WHERE {0} < (SELECT MIN({0}) FROM (SELECT TOP {1} {0} FROM {2} {3} ORDER BY {0} DESC) AS TMP) {4} ORDER BY {5} DESC", new object[] { str1, num, table, str2, str3, str });
                //}
            }
            if (isCount)
            {
                builder.AppendFormat(";SELECT COUNT({0}) FROM {1} {2}", pk, table, str2);
            }
            return builder.ToString();
        }
