...
{
    public class ListDistributorSales : AdminPage
    {
        //页面加载
        ...
        /// 绑定店员列表
        /// </summary>
        private void BindDistributorSales()
        {
            int disuserid = 0;
        ...
        {
            Guid Id =new Guid(this.grdProductTypes.DataKeys[e.RowIndex].Value.ToString());

            if (DistributorSalesHelper.DeleteStoreInfo(Id))
            {
                this.BindDistributorSales();
                this.ShowMsg("成功删除了一个店员信息", true);
            }
            else
            {
                this.ShowMsg("删除店员信息失败", false);
            }
        }
