...
{
    public class ListSupplier : AdminPage
    {
        ...
        private void judge()
        {
            ...
            {
                //权限ID
                RoleId=masterSettings.SupplierRoleId,
                //供应商ID
                ClientUserId=Convert.ToInt32(Request["gysid"])
            };
            ...
            if (dtmg.Rows.Count > 0)
            {
                ...
                Response.Write("/admin/store/EditManager.aspx?gysid=" + Request["gysid"] + "&userid=" + userid);
                Response.End();
            }
            else
            {
                Response.Write("/admin/store/AddManager.aspx?gysid=" + Request["gysid"]);
                Response.End();
            }
        ...
        {
            ListSupplierQuery query = new ListSupplierQuery { gysName = this.searchkey, PageIndex = this.pager.PageIndex, PageSize = this.pager.PageSize };
           
            DbQueryResult listSupplier=SupplierHelper.GetListSupplier(query);
            this.grdProductTypes.DataSource = listSupplier.Data;
            this.grdProductTypes.DataBind();
            this.pager.TotalRecords = listSupplier.TotalRecords;
        }
        ...
        private void grdProductTypes_RowDeleting(object sender, System.Web.UI.WebControls.GridViewDeleteEventArgs e)
        {
            ...
            if (SupplierHelper.DeleteSupplier(Id))
            {
                //删除后台用户
                //ClientUserId = Id
                //RoleId = 6
