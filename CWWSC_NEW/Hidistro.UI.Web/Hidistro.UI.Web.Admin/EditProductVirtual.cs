...
namespace Hidistro.UI.Web.Admin
{
    ...
    public class EditProductVirtual : ProductBasePage
    {
        ...
        protected System.Web.UI.WebControls.RadioButton radOnAuditing;//待审核
        ...
        private void btnSave_Click(object sender, System.EventArgs e)
        {
            ...
            if (this.ValidateConverts(this.chkSkuEnabled.Checked, out num, out num2, out neigouprice, out nullable, out nullable2, out neigouNum, out num3, out nullable3, out commvalue1, out commvalue2, out commvalue3, out commvalue4, out commvalue5, out commvalue6))
            {
                ...
                ProductInfo target = ProductHelper.GetProductDetails(this.productId, out dictionary, out tagsId);
                ...
                target.IsfreeShipping = true;//虚拟商品默认包邮
                ...
                {
                    target.HasSKU = true;
                    skus = base.GetVirtualSkus(this.txtSkus.Text);
                }
                else
                {
                    /***满足保留库存**/
                    int numstock = 0;
                    if (target.Skus.Count > 0)
                    {
                        foreach (string str in target.Skus.Keys)
                        {
                            numstock = target.Skus[str].Stock;
                        }
                    }
                    /*****/
                ...
                else
                {
                    ...
                    switch (ProductHelper.UpdateProduct(target, skus, attrs, tagIds))
                    {
                        case ProductActionStatus.Success:
                            ...
                            this.ShowMsgAndReUrl("修改商品成功", true, "ProductOnVirtual.aspx");
        ...
        private void LoadProduct(ProductInfo product, System.Collections.Generic.Dictionary<int, System.Collections.Generic.IList<int>> attrs)
        {
            ...
            ////酷开商品内码,并且酷开商品内码无法修改
            //this.txtKuKaiProductCode.Text = product.KukaiCode;
            ...
            {
                this.RadioWholesaleA.Checked = true;
            }
            else
            {
                this.RadioWholesaleB.Checked = true;
            }
            if (product.wholesaleCommisionRatio > 0)
                this.txtWholesaleCommisionRatio.Text = product.wholesaleCommisionRatio.ToString("F2");
            if (product.wholesaleCommisionMoney > 0)
                this.txtWholesaleCommisionMoney.Text = product.wholesaleCommisionMoney.ToString("F2");
            ...
            if (!this.Page.IsPostBack)
            {
                ...
                else
                {
                    if (!string.IsNullOrEmpty(base.Request.QueryString["categoryId"]))
                    {
                        ...
                        this.lnkEditCategory.NavigateUrl = "SelectCategoryVirtual.aspx?categoryId=" + this.categoryId.ToString(System.Globalization.CultureInfo.InvariantCulture);
                    }
                    else
                    {
                        ...
                        this.lnkEditCategory.NavigateUrl = "SelectCategoryVirtual.aspx?categoryId=" + product.CategoryId.ToString(System.Globalization.CultureInfo.InvariantCulture);
                    }
        ...
        private bool ValidateConverts(bool skuEnabled, out int displaySequence, out decimal salePrice, out decimal? neigouprice, out decimal? costPrice, out decimal? marketPrice, out int neigouNum, out int stock, out decimal? weight, out decimal? saleRatio, out decimal? saleMoney, out decimal? neigouRatio, out decimal? neigouMoney, out decimal? wholesaleRatio, out decimal? wholesaleMoney)
        {
            ...
            /**设置佣金**/
            if (this.RadioSaleA.Checked)//一口价佣金
            {
                decimal inum;
            ...
            if (this.RadioNeigouA.Checked)//内购价佣金
            {
                decimal inum;
            ...
            if (this.RadioWholesaleA.Enabled)
            {
                if (this.RadioWholesaleA.Checked)//批发价佣金
                {
                    ...
                    {
                        str += Formatter.FormatErrorMessage("请正确填写批发价返佣比例");
                    }
                    decimal inum1;
                ...
                else
                {
                    ...
                    {
                        str += Formatter.FormatErrorMessage("请正确填写批发价返佣金额");
                    }
                    decimal inum1;
