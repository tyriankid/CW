...
namespace Hidistro.UI.Web.Admin
{
	public class ProductBasePage : AdminPage
	{
				...
				else
				{
					...
					foreach (XmlNode node in list)
					{
						SKUItem item = new SKUItem
						{
                            ...
                            NeigouPrice = (node.Attributes["neigouPrice"].Value.Length > 0) ? decimal.Parse(node.Attributes["neigouPrice"].Value) : 0m,
        ...
        protected System.Collections.Generic.Dictionary<string, SKUItem> GetServiceSkus(string skusXml)
        {
            XmlDocument document = new XmlDocument();
            System.Collections.Generic.Dictionary<string, SKUItem> dictionary = null;
            System.Collections.Generic.Dictionary<string, SKUItem> result;
        ...
        protected System.Collections.Generic.Dictionary<string, SKUItem> GetVirtualSkus(string skusXml, Dictionary<string, SKUItem> oldskus = null)
        {
            XmlDocument document = new XmlDocument();
            System.Collections.Generic.Dictionary<string, SKUItem> dictionary = null;
            System.Collections.Generic.Dictionary<string, SKUItem> result;
            try
            {
                ...
                else
                {
                    ...
                    foreach (XmlNode node in list)
                    {
                        ...
                        if (oldskus != null)
                        {
                            foreach (string strkey in oldskus.Keys)
                            {
                                if (item.SkuId == oldskus[strkey].SkuId)
                                    numstock = oldskus[strkey].Stock;
                            }
                        }
                        if (numstock > 0)
                            item.Stock = numstock;
