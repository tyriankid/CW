namespace Hidistro.UI.Web.Admin
{
    ...
    public class EditGroupBuy : AdminPage
    {
        ...
        private void btnUpdateGroupBuy_Click(object sender, EventArgs e)
        {
            ...
            {
                groupBuy.EndDate = this.calendarEndDate.SelectedDate.Value.AddHours((double)this.dropEndHours.SelectedValue.Value).AddMinutes((double)this.dropEndMinute.SelectedValue.Value);
                ...
                else if (DateTime.Compare(this.calendarStartDate.SelectedDate.Value.AddHours((double)this.dropStartHours.SelectedValue.Value).AddMinutes((double)this.dropStartMinute.SelectedValue.Value), groupBuy.EndDate) >= 0)
                {
                    str = str + Formatter.FormatErrorMessage("开始日期必须要早于结束日期");
                }
                else
                {
                    groupBuy.StartDate = this.calendarStartDate.SelectedDate.Value.AddHours((double)this.dropStartHours.SelectedValue.Value).AddMinutes((double)this.dropStartMinute.SelectedValue.Value);
                }
            }
        ...
        private void LoadGroupBuy(GroupBuyInfo groupBuy)
        {
            ...
            this.dropStartHours.SelectedValue = new int?(groupBuy.StartDate.Hour);
            this.dropEndHours.SelectedValue = new int?(groupBuy.EndDate.Hour);
            this.dropStartMinute.SelectedValue = new int?(groupBuy.StartDate.Minute);
            this.dropEndMinute.SelectedValue = new int?(groupBuy.EndDate.Minute);
