...
namespace Hidistro.UI.Web.Admin
{
    ...
    public class EditProductService : ProductBasePage
    {
        protected string ReUrl = "productonservice.aspx";
        ...
        private void btnSave_Click(object sender, System.EventArgs e)
        {
            ...
            int neigouNum;//限购数量
            if (this.ValidateConverts(this.chkSkuEnabled.Checked, out num, out num2, out neigouprice, out nullable, out nullable2, out neigouNum, out num3, out nullable3))
            {
                ...
                target.IsfreeShipping = true;//服务商品默认设置包邮
                ...
                target.ClassId = this.dropClassList.SelectedValue;
                //来源及供应商
                target.ProductSource = 3;

                //酷开商品内码
                //target.KukaiCode = this.txtKuKaiProductCode.Text;
                ...
                else
                {
                    ...
                    switch (ProductHelper.UpdateProduct(target, skus, attrs, tagIds))
                    {
                        case ProductActionStatus.Success:
                            ...
                            //}
                            //if (this.DDLProductSource.SelectedValue == "2")
                            this.ReUrl = "ProductOnService.aspx";
                            this.ShowMsgAndReUrl("修改服务商品成功", true, this.ReUrl);
                            this.ShowMsg("修改服务商品成功", true);
                            break;
                        case ProductActionStatus.DuplicateName:
                            this.ShowMsg("修改服务商品失败，商品名称不能重复", false);
                            break;
                        case ProductActionStatus.DuplicateSKU:
                            this.ShowMsg("修改服务商品失败，商家内码不能重复", false);
                            break;
                        case ProductActionStatus.SKUError:
                            this.ShowMsg("修改服务商品失败，商家内码不能重复", false);
                            break;
                        case ProductActionStatus.AttributeError:
                            this.ShowMsg("修改服务商品失败，保存商品属性时出错", false);
                            break;
                        case ProductActionStatus.OffShelfError:
                            this.ShowMsg("修改服务商品失败， 子站没在零售价范围内的商品无法下架", false);
                            break;
                        case ProductActionStatus.ProductTagEroor:
                            this.ShowMsg("修改服务商品失败，保存商品标签时出错", false);
                            break;
                        default:
                            this.ShowMsg("修改服务商品失败，未知错误", false);
                            break;
                    }
                }
            }
        ...
        private void LoadProduct(ProductInfo product, System.Collections.Generic.Dictionary<int, System.Collections.Generic.IList<int>> attrs)
        {
            ...
            this.dropClassList.SelectedValue = product.ClassId;//服务品类
        ...
        private bool ValidateConverts(bool skuEnabled, out int displaySequence, out decimal salePrice, out decimal? neigouprice, out decimal? costPrice, out decimal? marketPrice, out int neigouNum, out int stock, out decimal? weight)
        {
