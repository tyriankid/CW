...
namespace Hidistro.UI.Web.Admin
{
    ...
    public class ProductLineOnDeleted : AdminPage
	{
		...
		private void BindProducts()
		{
			...
			ProductQuery entity = new ProductQuery
			{
                ...
                ProductSource =4,//线下商品
                FilialeId = isFiliale?currentManager.ClientUserId:0,
			}   ;
		...
		private void btnUpShelf_Click(object sender, System.EventArgs e)
		{
			...
			if (string.IsNullOrEmpty(str))
			{
				this.ShowMsg("请先选择要还原的商品", false);
			}
			else
			{
                if (ViewState["isFiliale"] != null)
                {
                    bool isFiliale = (bool)ViewState["isFiliale"];
                    if (isFiliale)
                    {
                        //分公司还原
                        if (ProductHelper.UpLineProduct(str,ProductSaleStatus.OnSale) > 0)
                        {
                            this.ShowMsg("成功还原了选择的商品，您可以在商品列表里面找到还原的商品", true);
                            this.BindProducts();
                        }
                        else
                            this.ShowMsg("还原商品失败，未知错误", false);
                    }
                    else
                    { 
                        //创维管理员还原
                        if (ProductHelper.UpLineProduct(str, ProductSaleStatus.line) > 0)
                        {
                            this.ShowMsg("成功上架了选择的商品，您可以在出售中的商品里面找到还原以后的商品", true);
                            this.BindProducts();
                        }
                        else
                            this.ShowMsg("还原商品失败，未知错误", false);
