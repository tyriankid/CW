...
namespace Hidistro.UI.Web.Admin
{
	...
	public class Roles : AdminPage
	{
        ...
        private void grdGroupList_RowDeleting(object sender, System.Web.UI.WebControls.GridViewDeleteEventArgs e)
        {
            ...
            int roleid = (int)this.grdGroupList.DataKeys[e.RowIndex].Value;
            if (roleid == 1)
            {
                this.ShowMsg("该角色为业务必须值，无法删除", true);
                return;
            }
            if (roleid == masterSettings.StoreRoleId)
            {
                this.ShowMsg("该角色为业务必须值，无法删除", true);
                return;
            }
            if (roleid == masterSettings.SupplierRoleId)
            {
                this.ShowMsg("该角色为业务必须值，无法删除", true);
                return;
            }
            if (roleid == masterSettings.FilialeRoleId)
            {
                this.ShowMsg("该角色为业务必须值，无法删除", true);
                return;
            }
            if (ManagerHelper.DeleteRole(roleid))
            {
                this.BindUserGroup();
