...
namespace Hidistro.UI.Web.Admin
{
	...
	public class ProductOnDeleted : AdminPage
	{
		...
		private void BindProducts()
		{
            ...
            {
                this.btnUpShelf.Text = "还原到商品列表中";
                isSupplier = true;
            }
            ViewState["isSupplier"] = isSupplier;
			this.LoadParameters();
			ProductQuery entity = new ProductQuery
			{
                ...
                ProductSource = isSupplier ? 2 : 1,//商品来源，1为创维、2为供应商  这里如果是供应商则显示供应商商品，如果是平台管理员则显示创维与供应商的商品
		...
		private void btnUpShelf_Click(object sender, System.EventArgs e)
		{
			...
			{
                if (ViewState["isSupplier"] != null)
                {
                    bool isSupplier = (bool)ViewState["isSupplier"];
                    if (isSupplier)
                    {
                        //供应商还原
                        if (ProductHelper.UpSupplierShelf(str) > 0)
                        {
                            this.ShowMsg("成功还原了选择的商品，您可以在商品列表里面找到待审核的商品", true);
                            this.BindProducts();
                        }
                        else
