namespace Hidistro.UI.Web.Admin
{
    ...
    public class EditCutDown : AdminPage
    {
        ...
        protected TextBox TxtCurrentPrice;//起始金额
        ...
        private void btnUpdateCutDown_Click(object sender, EventArgs e)
        {
            ...
            {
                cutDown.EndDate = this.calendarEndDate.SelectedDate.Value.AddHours((double)this.dropEndHours.SelectedValue.Value).AddMinutes((double)this.dropEndMinute.SelectedValue.Value);
                ...
                else if (DateTime.Compare(this.calendarStartDate.SelectedDate.Value.AddHours((double)this.dropStartHours.SelectedValue.Value).AddMinutes((double)this.dropStartMinute.SelectedValue.Value), cutDown.EndDate) >= 0)
                {
                    str = str + Formatter.FormatErrorMessage("开始日期必须要早于结束日期");
                }
                else
                {
                    cutDown.StartDate = this.calendarStartDate.SelectedDate.Value.AddHours((double)this.dropStartHours.SelectedValue.Value).AddMinutes((double)this.dropStartMinute.SelectedValue.Value);
                }
            }
            //每次砍价金额
        ...
        private void LoadCutDown(CutDownInfo cutDown)
        {
            ...
            this.TxtCurrentPrice.Text = cutDown.CurrentPrice.ToString("F2");
            ...
            this.dropStartHours.SelectedValue = new int?(cutDown.StartDate.Hour);
            this.dropEndHours.SelectedValue = new int?(cutDown.EndDate.Hour);
            this.dropStartMinute.SelectedValue = new int?(cutDown.StartDate.Minute);
            this.dropEndMinute.SelectedValue = new int?(cutDown.EndDate.Minute);
