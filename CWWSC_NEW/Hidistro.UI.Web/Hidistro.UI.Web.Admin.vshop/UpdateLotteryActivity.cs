...
namespace Hidistro.UI.Web.Admin.vshop
{
	public class UpdateLotteryActivity : AdminPage
	{
        ...
        protected HourDropDownList dropStartHours;//开始小时
        protected HourDropDownList dropEndHours;//结束小时
        ...
        protected System.Web.UI.WebControls.TextBox txtIntegral4;//积分值
        protected System.Web.UI.WebControls.TextBox txtIntegral5;//积分值
        protected System.Web.UI.WebControls.TextBox txtIntegral6;//积分值

        protected System.Web.UI.WebControls.CheckBox checkJf4;//是否积分
        protected System.Web.UI.WebControls.CheckBox checkJf5;//是否积分
        protected System.Web.UI.WebControls.CheckBox checkJf6;//是否积分

		private int type;
        protected System.Web.UI.WebControls.HiddenField hiActivityType;
					...
					else
					{
						...
						else
						{
							LotteryActivityInfo info = new LotteryActivityInfo
							{
                                ...
                                StartTime = this.calendarStartDate.SelectedDate.Value.AddHours((double)this.dropStartHours.SelectedValue.Value),
                                EndTime = this.calendarEndDate.SelectedDate.Value.AddHours((double)this.dropEndHours.SelectedValue.Value),
							...
							if (int.TryParse(this.txtPrize1Num.Text, out num2) && int.TryParse(this.txtPrize2Num.Text, out num2) && int.TryParse(this.txtPrize3Num.Text, out num2))
							{
								...
								if (this.ChkOpen.Checked)
								{
                                    ...
                                    if (this.checkJf4.Checked)
                                    {
                                        if (!int.TryParse(this.txtIntegral4.Text, out num2) || num2 <= 0)
                                        {
                                            this.ShowMsg("奖励积分必须为大于0的整数！", false);
                                            return;
                                        }
                                    }
                                    if (this.checkJf5.Checked)
                                    {
                                        if (!int.TryParse(this.txtIntegral5.Text, out num2) || num2 <= 0)
                                        {
                                            this.ShowMsg("奖励积分必须为大于0的整数！", false);
                                            return;
                                        }
                                    }
                                    if (this.checkJf6.Checked)
                                    {
                                        if (!int.TryParse(this.txtIntegral6.Text, out num2) || num2 <= 0)
                                        {
                                            this.ShowMsg("奖励积分必须为大于0的整数！", false);
                                            return;
                                        }
									...
									PrizeSetting setting4 = new PrizeSetting
									{
										...
										PrizeLevel = "四等奖",
										Probability = num6,
                                        Integral = this.checkJf4.Checked ? System.Convert.ToInt32(this.txtIntegral4.Text) : 0
									};
									list2.Add(setting4);
									PrizeSetting setting5 = new PrizeSetting
									{
										...
										PrizeLevel = "五等奖",
										Probability = num7,
                                        Integral = this.checkJf5.Checked ? System.Convert.ToInt32(this.txtIntegral5.Text) : 0
									};
									list2.Add(setting5);
									PrizeSetting setting6 = new PrizeSetting
									{
										...
										PrizeLevel = "六等奖",
										Probability = num8,
                                        Integral = this.checkJf6.Checked ? System.Convert.ToInt32(this.txtIntegral6.Text) : 0
									};
									list2.Add(setting6);
									list.AddRange(list2);
								}
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			else
			{
				...
				case 3:
					...
					break;
                case 6:
                    this.LitTitle.Text = "拆礼盒编辑";
                    this.Litdesc.Text = "拆礼盒编辑";
                    break;
                case 9:
                    this.LitTitle.Text = "微信红包编辑";
                    this.Litdesc.Text = "微信红包编辑";
                    break;
				}
				if (!this.Page.IsPostBack)
				{
					...
					this.RestoreLottery();
                    hiActivityType.Value = this.type.ToString();
				}
			}
		}
		public void RestoreLottery()
		{
            ...
            //设置选中的小时部分
            this.dropStartHours.SelectedValue = lotteryActivityInfo.StartTime.Hour;
            this.dropEndHours.SelectedValue = lotteryActivityInfo.EndTime.Hour;
			...
			if (lotteryActivityInfo.PrizeSettingList.Count > 3)
			{
                ...
                if (lotteryActivityInfo.PrizeSettingList[3].Integral != null && lotteryActivityInfo.PrizeSettingList[3].Integral > 0)
                {
                    this.checkJf4.Checked = true;
                    this.txtIntegral4.Text = lotteryActivityInfo.PrizeSettingList[3].Integral.ToString();
                }
                ...
                if (lotteryActivityInfo.PrizeSettingList[4].Integral != null && lotteryActivityInfo.PrizeSettingList[4].Integral > 0)
                {
                    this.checkJf5.Checked = true;
                    this.txtIntegral5.Text = lotteryActivityInfo.PrizeSettingList[4].Integral.ToString();
                }
                ...
                if (lotteryActivityInfo.PrizeSettingList[5].Integral != null && lotteryActivityInfo.PrizeSettingList[5].Integral > 0)
                {
                    this.checkJf6.Checked = true;
                    this.txtIntegral6.Text = lotteryActivityInfo.PrizeSettingList[5].Integral.ToString();
                }
			}
		}
	}
