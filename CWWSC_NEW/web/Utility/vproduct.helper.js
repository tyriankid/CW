...
function BuyProduct() {
    ...
    $.ajax({
        url: "/API/VshopProcess.ashx",
        type: 'post', dataType: 'json', timeout: 10000,
        data: { action: "ValidateBuyNum", productId: $("#vProductDetails_litproductid").val(), quantity: parseInt($("#buyNum").val()), productSkuId: $("#hiddenSkuId").val(), from: type },
        async: false,
        success: function (resultData) {
            if (resultData.Status == "YES") {
                ...
                else if (resultData.ProSource == "5")
                    location.href = "/Vshop/SubmmitOrderVirtual.aspx?buyAmount=" + $("#buyNum").val() + "&productSku=" + $("#hiddenSkuId").val() + "&from=" + type + "&" + idName + "=" + $('#litGroupbuyId').val();
                else
...
function BuyProductToCart() {
    $.ajax({
        url: "/API/VshopProcess.ashx",
        type: 'post', dataType: 'json', timeout: 10000,
        data: { action: "AddToCartBySkus", quantity: parseInt($("#buyNum").val()), productSkuId: $("#hiddenSkuId").val(), categoryid: $("#vProductDetails_litCategoryId").val() },
        async: false,
        success: function (resultData) {
