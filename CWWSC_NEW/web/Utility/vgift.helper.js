...
function BuyProduct() {
    ...
    if ($("#vGiftDetails_litgiftid").val() == null || $("#vGiftDetails_litgiftid").val() == "")
    {
        alert_h("商品不存在，请返回重新加载并兑换。");
        return false;
    }
    $.ajax({
        url: "/API/VshopProcess.ashx",
        type: 'post', dataType: 'json', timeout: 10000,
        data: { action: "validatePoints", giftId: $("#vGiftDetails_litgiftid").val(), buyNum: quantity },
        success: function (resultData) {
            if (resultData.Status == "YES") {
                location.href = "/Vshop/SubmmitOrder.aspx?giftid=" + $("#vGiftDetails_litgiftid").val() + "&buyAmount=" + $("#buyNum").val() + "&isGift=1&from=signBuy";
            } else {
