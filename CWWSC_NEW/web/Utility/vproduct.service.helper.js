...
function SelectSkus(clt) {
    ...
    //如果是会员充值增送优惠券通道,则不需要计算会员价
    var isMemberPrice = true;
    if ($("#specialHideShow").val() == "JZ") { isMemberPrice = false; }
    // 保存当前选择的规格
    ...
    if (allSelected) {
        ...
        $.ajax({
            ...
            data: { action: "GetSkuByOptions", productId: $("#hiddenProductId").val(), options: selectedOptions, IsMemberPrice: isMemberPrice },
...
function BuyProduct() {
    ...
    if (url.indexOf("countdownproductdetail.aspx") > -1 || url.indexOf("countdownproduct_detail") > -1) {
        type = "countDown";
        idName = "countDownId";
    }
    ...
    $.ajax({
        url: "/API/VshopProcess.ashx",
        type: 'post', dataType: 'json', timeout: 10000,
        data: { action: "ValidateBuyNum", productId: $("#vProductServiceDetails_litproductid").val(), quantity: parseInt($("#buyNum").val()), productSkuId: $("#hiddenSkuId").val(), from: type },
        async: false,
        success: function (resultData) {
            if (resultData.Status == "YES") {
                if (resultData.ProSource == "3")
                    location.href = "/Vshop/SubmmitOrderService.aspx?buyAmount=" + $("#buyNum").val() + "&productSku=" + $("#hiddenSkuId").val() + "&from=" + type + "&" + idName + "=" + $('#litGroupbuyId').val();
                else
                    location.href = "/Vshop/SubmmitOrder.aspx?buyAmount=" + $("#buyNum").val() + "&productSku=" + $("#hiddenSkuId").val() + "&from=" + type + "&" + idName + "=" + $('#litGroupbuyId').val();
            }
            if (resultData.Status == "NO") {
                myConfirm('询问', '内购价只允许购买 ' + resultData.Num + '件，您已经无法享受内购价，是否原价购买吗？', '确定购买', function () {
                    //alert_h("内购门店商品只允许购买 " + resultData.Num + " 件！");
                    if (resultData.ProSource == "3")
                        location.href = "/Vshop/SubmmitOrderService.aspx?buyAmount=" + $("#buyNum").val() + "&productSku=" + $("#hiddenSkuId").val() + "&from=" + type + "&" + idName + "=" + $('#litGroupbuyId').val();
                    else
                        location.href = "/Vshop/SubmmitOrder.aspx?buyAmount=" + $("#buyNum").val() + "&productSku=" + $("#hiddenSkuId").val() + "&from=" + type + "&" + idName + "=" + $('#litGroupbuyId').val();
...
function BuyProductToCart() {
    $.ajax({
        url: "/API/VshopProcess.ashx",
        type: 'post', dataType: 'json', timeout: 10000,
        data: { action: "AddToCartBySkus", quantity: parseInt($("#buyNum").val()), productSkuId: $("#hiddenSkuId").val(), categoryid: $("#vProductServiceDetails_litCategoryId").val() },
        async: false,
        success: function (resultData) {
		...
		//alert(resultData.Status);
                if (resultData.Status == "2") {
                    //location.href = "/Vshop/UserLogin.aspx?userstatus=0&productid=" + $("#vProductServiceDetails_litproductid").val();
                alert_h("请先登录系统", function () {
                        location.href = "/Vshop/MemberCenter.aspx?userstatus=0&productid=" + $("#vProductServiceDetails_litproductid").val();
                    });
                }
                else if (resultData.Status == "-1") {
                    // 抛出异常消息
