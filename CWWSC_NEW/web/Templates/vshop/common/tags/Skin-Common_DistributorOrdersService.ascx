...
<div class="member-orders-content  member-ordersli-content">
  <!-- 商品循环区-->
        ...
        <ItemTemplate>
             <div class="member-orders-item">
                  ...
                  <div class="info">
                      ...
                      <div class="member-right-box" style="display:none">
   ...
   <!--商品循环区end-->
    ...
    <!-- 订单信息区(下方)-->
            <div class="link-box" >
                <span style="font-size: 12px; padding-right:10px">用户昵称：<%# Eval("UserName")%></span><em class="red"><Hi:OrderStatusLabelService ID="OrderStatusLabelService2" OrderStatusCode='<%# Eval("OrderStatus") %>' runat="server" /></em> 
                <asp:Literal runat="server" ID="litSendOrderGoods" Visible="false"></asp:Literal>
                <span><a href='<%# Globals.ApplicationPath + "/Vshop/LookServiceSales.aspx?OrderId=" + Eval("OrderId") + "&OrderStatus=" + Eval("OrderStatus") %>' class='link <%# ((int)Eval("OrderStatus") == 3 || (int)Eval("OrderStatus") == 5) ? "link" : "hide"%>'>查看配单信息</a></span>
            </div>
