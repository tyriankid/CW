  ...
  <!-- 商品循环区-->
   ...
   <!--商品循环区end-->
    ...
    <!-- 按钮动态显示区-->
            ...
            <div class="link-box">
                <em class="red"><Hi:OrderStatusLabelService ID="OrderStatusLabelService2" OrderStatusCode='<%# Eval("OrderStatus") %>' runat="server" /></em> 
                <a href='<%# Globals.ApplicationPath + "/Vshop/MyOrderSellerService.aspx?OrderId=" + Eval("OrderId") %>' class='link <%# ((int)Eval("OrderStatus") > 1) ? "link" : "hide"%>'>售后-客户信息</a>
                <a href='<%# Globals.ApplicationPath + "/Vshop/OrderServiceCheck.aspx?OrderId=" + Eval("OrderId") %>' class='link <%# ((int)Eval("OrderStatus") == 3) ? "link link-color" : "hide"%>'>核销订单</a>

                <%--<a href='<%# Globals.ApplicationPath + "/Vshop/LookServiceSales.aspx?OrderId=" + Eval("OrderId") %> '
                                        class='link <%# ((int)Eval("OrderStatus") == 3 || (int)Eval("OrderStatus") == 5) ? "link" : "hide"%>'>查看配单信息</a>--%>

                <%--<a href='javascript:void(0)'onclick="CloseOrder('<%#Eval("OrderId") %>')" class='link <%# ((int)Eval("OrderStatus") == 1) ? "link" : "hide"%>'>关闭订单</a>
                <a href='javascript:void(0)'onclick="CloseOrder('<%#Eval("OrderId") %>')" class='link <%# ((int)Eval("OrderStatus") == 2) ? "link" : "hide"%>'>配单</a>
