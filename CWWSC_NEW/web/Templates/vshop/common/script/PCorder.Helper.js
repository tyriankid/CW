...
function getTotalPrice() {
    ...
    $("#orderList ul li").each(function () {
        /*tPrice = getSum(tPrice, getX($(this).find("b").html(), $(this).find("input[type=tel]").attr("value")));*/
        tPrice = getSum(tPrice, getX($(this).find("b").html(), $(this).find("input[type=tel]").attr("value"), $(this).find("#giveNum").attr("value")));
    });
    //计算优惠券优惠
...
function getX(num1, num2, num3) {
    if (num3 == null || num3 == "") {
        num3 = "0";
    }
    return roundFun((parseFloat(num1) * (parseFloat(num2) - parseFloat(num3))), 2);
}
