...
<%@ Register TagPrefix="cc1" TagName="Order_Receipt" Src="~/Admin/Ascx/Order_Receipt.ascx" %>
...
<div class="Purchase">
      <div class="Settlement">
        ...
        <td>
          <ul class="order_ul">
        	<li></li>
            <li class="order_li">
                <span>昵称：<asp:Literal runat="server" ID="litUserName" /></span>
                <span>真实姓名：<asp:Literal runat="server" ID="litRealName" /></span>
                <span>联系电话：<asp:Literal runat="server" ID="litUserTel" /></span>
                <span>电子邮件：<asp:Literal runat="server" ID="litUserEmail" /></span>
            </li>
            <li class="order_li">
                <span><asp:Literal ID="litPayTime"  runat="server" /></span>
                <span><asp:Literal ID="litSendGoodTime"  runat="server" /></span>
                <span><asp:Literal ID="litFinishTime"  runat="server" /></span>
            </li>
            <li class="Pg_8"><span style="display:none"><asp:HyperLink runat="server" ID="lkbtnEditPrice" class="link_default_blue" Text="修改价格" /></span>
                <span><a class="link_default_orange" href="javascript:ShowRemarkOrder();">备注</a></span>
                 <span><a class="link_default_gray" id="lbtnClocsOrder" runat="server" href="javascript:ShowCloseOrder();">关闭订单</a></span>
  ...
  <div class="blank12 clearfix"></div>
   ...
   <cc1:Order_Receipt  runat="server" ID="Receipt" />
...
<asp:Content ID="Content3" ContentPlaceHolderID="validateHolder" runat="server">
    <script type="text/javascript">
        $(function(){
            window.setTimeout(function getNewExpress()
            {
                var $expressInfo = $("#expressInfo");
                var $no=$("#companyName").parents().find("tr").next().next().next().find("td").eq(1);
                var com=$("#companyName").html();
                var nu=$no.html().substr($no.html().indexOf("：")+1);
                if($expressInfo.html()==undefined){//如果订单未提交,则不做任何处理
                    return;
                }
                if($expressInfo.html().indexOf("该单号暂无物流进展，请稍后再试，或检查公司和单号是否有误。")>-1 || expressInfo.html().indexOf("正在加载中") > -1)
                {
                    $expressInfo.html('');
                    //var kuaidiUrl="http://www.kuaidi100.com/chaxun?com={0}&nu={1}";
                    var kuaidiUrl = "http://m.kuaidi100.com/index_all.html?type={0}&postid={1}";
                    var iframeHtml="<iframe id='iframeMain' width='60%' height='700px' src='"+kuaidiUrl+"'></iframe> ";
                    iframeHtml = iframeHtml.replace("{0}",com);
                    iframeHtml = iframeHtml.replace("{1}",nu);
                    $("#iframeExpress100").html(iframeHtml);
                }
                else
                {
                    //alert(2);
