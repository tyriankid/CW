                ...
                <ItemTemplate>
                    <tr class="td_bg noboer_tr">
                        <td>
                            <input name="CheckBoxGroup" type="checkbox" value='<%#Eval("OrderId") %>' />订单编号：<%#Eval("OrderId") %><asp:Literal
                                ID="group" runat="server" Text='<%#  Eval("GroupBuyId")!=DBNull.Value&& Convert.ToInt32(Eval("GroupBuyId"))>0?"(团)":"" %>' /><span style="display:<%#Eval("GiveQuantity").ToString()=="1"?"":"none" %>">买一送一</span>
                        </td>
                    ...
                    <tr class="td_bg">
                        ...
                        <td style="font-weight: bold; font-family: Arial;">
                            订单同步： <%#Eval("submitgk").ToString() == "1" ? "已同步" : "未同步" %><br />
                        ...
                        <td class="td_txt_cenetr">
                           &nbsp;<span class="Name" style="text-indent:0px;">
                                ...
                                <asp:Label CssClass="submit_faihuo" ID="lkbtnSendGoods" Visible="false" runat="server" > 
                                    ...
                                    <%#Eval("OrderSource").ToString() == "1" ? "等待港口发货" : "<a href=\"javascript:DialogFrame('sales/SendOrderGoods.aspx?OrderId="+Eval("OrderId")+"&reurl=encodeURIComponent(goUrl)','订单发货',null,null, function () { location.href=goUrl })\">发货</a>" %>
                                </asp:Label>
                                
                                <br />
                                <a class="link_default_gray" name="link_default_gray" href="javascript:SendGK('<%#Eval("OrderId") %>');"><%#(Eval("OrderStatus").ToString() == "2" && Eval("submitgk").ToString() == "0") ? "推送港口" : "" %></a>
...
<asp:Content ID="Content3" ContentPlaceHolderID="validateHolder" runat="server">
    ...
    <script type="text/javascript">
        ...
        //重新推送
        function SendGK(orderId) {
            var r = confirm("确认要重新推送吗？");
            if (r == true) {
                var url = '/API/VshopProcess.ashx';
                $.ajax({
                    type: "get",
                    url: url,
                    data: { action: 'sendgk', orderId: orderId },
                    dataType: "json",
                    async: false,
                    success: function (data) {
                        if (data.success == "true") {
