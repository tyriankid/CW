...
{
    public partial class Refundservice : AdminPage
	{
        ...
        private ReturnsApplyQuery GetQuery()
		{
            ...
            //2017-07-28 服务商品流程增加内容 验证当前登陆用户类型
            bool isStore = false;//是否门店，是则显示服务门店退货订单
            ManagerInfo currentManager = ManagerHelper.GetCurrentManager();
            SiteSettings masterSettings = SettingsManager.GetMasterSettings(false);
            if (currentManager.RoleId == masterSettings.StoreRoleId)
            {
                isStore = true;//当前登陆为门店用户
            }
            returnsApplyQuery.OrderSource = 3;//服务订单
            if (isStore)
                returnsApplyQuery.ServiceUserId = currentManager.ClientUserId;
