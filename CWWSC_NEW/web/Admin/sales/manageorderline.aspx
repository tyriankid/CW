<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true"
    CodeBehind="manageorderline.aspx.cs" Inherits="Hidistro.UI.Web.Admin.manageorderline" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="contentHolder" runat="server">
    ...
    <div class="dataarea mainwidth">
        ...
        <div class="searcharea clearfix br_search search_titxt">
            <ul>
                <li><span>时 间 段：</span>
                    <span>
                        <UI:WebCalendar CalendarType="StartDate" ID="calendarStartDate" runat="server" CssClass="forminput" /></span>
                    <span class="Pg_1010">至</span>
                    <span>
                        <UI:WebCalendar ID="calendarEndDate" runat="server" CalendarType="EndDate" CssClass="forminput" /></span>
                    ...
                    <span id="agentPpurchase" runat="server">
                        ...
                        <span>&nbsp;&nbsp;代理商昵称：</span> <span>
                            <asp:TextBox ID="txtRealName" runat="server" CssClass="forminput" /></span>
                    </span>
                </li>
                <li>
                    <span>金力账号：</span>
                    <span>
                        <asp:TextBox ID="txtAllHereCode" runat="server" CssClass="forminput" /></span>
                </li>
                ...
                <%--<li><span>商品来源：</span><span>
                    <asp:DropDownList ID="ddlOrderSource" runat="server" Width="100">
                            <asp:ListItem Text="全部" Value="0"></asp:ListItem>
                            <asp:ListItem Text="内部创维" Value="1"></asp:ListItem>
                            <asp:ListItem Text="外部供应商" Value="2"></asp:ListItem>
                        </asp:DropDownList>
                </span></li>--%>
                ...
                <li id="txtFilialeName">
                    <span>分公司名称:</span>
                    <asp:TextBox ClientIDMode="Static" Style="height: 20px; padding: 4px 0; width: 400px" ID="txtFiliale" oninput="inputchange(this)" onfocus="inputgetfocus(this)" onblur="inputlosefocus(this)" runat="server" />
                    <ul style="width: 400px; position: absolute; border: 1px solid #cecaca;display:none; background: #fff; left: 5rem; margin-top: 0rem; height: 300px; overflow-y: auto; z-index: 999" id="bcul">
                        <asp:Repeater runat="server" ID="ReFilialeIdOrder">
                            <ItemTemplate>
                                <li>
                                    <span name="<%#Eval("id") %>" onclick="selectde(this)" style="height: 1rem; line-height: 1rem; margin: 0; padding: 0.25rem 0; cursor: pointer"><%#Eval("fgsName") %></span></li>
                            </ItemTemplate>
                        </asp:Repeater>
                    </ul>
                </li>
                ...
                <li style="display: none"><span>快递公司：</span><span>
            ...
            <div class="batchHandleArea">
                <ul>
                    <li class="batchHandleButton">
                        ...
                        <%--<span class="sendproducts"><a href="javascript:changeOrder()"  onclick="">退货</a></span> --%>
                    </li>
                    <li class="batchHandleButton" style="width: 100%; margin-top: 10px; display: none;">
                        <span class="sendproducts">
                            <asp:Button runat="server" ID="btnPack" OnClientClick="return pack()" Text="订单导出" Style="display: block; background: transparent; line-height: 22px; border: none; outline: none; color: #fff;"></asp:Button></span>
                        <span style="position: relative; line-height: 22px; overflow: hidden;">
                            <asp:FileUpload ID="fileOrderInfoPack" runat="server" Style="position: absolute; left: 0; top: 0; opacity: 0; filter: alpha(opacity=0); -webkit-opacity: 0; z-index: 1;" />
                            点击这里上传文件</span>
                        <span class="sendproducts">
                            <asp:Button runat="server" ID="btnUnPack" Text="订单导入" Style="display: block; background: transparent; line-height: 22px; border: none; outline: none; color: #fff;"></asp:Button></span>
                        <span class="sendproducts" style="display: <%=Hidistro.ControlPanel.Config.CustomConfigHelper.Instance.IsJiuZhouTong?"none":"" %>">
                            <asp:Button runat="server" ID="btnMail" Text="打印电子面单" OnClientClick="return Mail()" Style="display: block; background: transparent; line-height: 22px; border: none; outline: none; color: #fff;"></asp:Button></span>
        ...
        <input type="hidden" id="hidFilialeOrder" runat="server"  clientidmode="Static"/><!--2017-8-2 控制分公司与总部的操作权限-->
        <input type="hidden" runat="server" id="hidFilialeId" clientidmode="Static" />
        <!--数据列表区域-->
        <div class="datalist">
                ...
                <HeaderTemplate>
                    <table width="0" border="0" cellspacing="0">
                        <tr class="table_title">
                            <td width="20%" class="td_right td_left">昵称
                            </td>
                            <td width="20%" class="td_right td_left">收货人
                            </td>
                            <td width="20%" class="td_right td_left">配送信息
                            </td>
                            <td width="110px" class="td_right td_left">支付方式
                            </td>
                            <td width="14%" class="td_right td_left">订单实收款(元)
                            </td>
                            <td width="170px" class="td_right td_left">订单状态
                            </td>
                            <td width="170px" class="td_left td_right_fff">操作
                            </td>
                        </tr>
                </HeaderTemplate>
                <ItemTemplate>
                    <tr class="td_bg noboer_tr">
                        ...
                        <td style="color: #666;">
                            <%#Eval("ReferralUserId").ToString()=="" ? "订单来源：主站" : Eval("ReferralUserId").ToString()=="0" ? "订单来源：主站" : "订单来源：<a href=\"javascript:ShowOrderSource('"+Eval("OrderId")+"');\">" + Eval("StoreName") + "</a>"%>
                        </td>
                    ...
                    <tr class="td_bg">
                        <td>&nbsp; <a href="<%# Eval("UserId").ToString()!="1100"?"javascript:DialogFrame('"+Globals.GetAdminAbsolutePath(string.Format("/member/MemberDetails.aspx?userId={0}",Eval("UserId")))+"','查看会员',null,null)":"javascript:void(0)" %>">昵称：<%#Eval("UserName")%></a><Hi:WangWangConversations runat="server" ID="WangWangConversations" WangWangAccounts='<%#Eval("Wangwang") %>' />
                            &nbsp;姓名：<%#Eval("RealName") %></td>
                        <td>收件人：<%#Eval("ShipTo") %>&nbsp;&nbsp;电话：<%#Eval("CellPhone") %></td>
                        <td>
                            <!--配送信息-->
                            地址：<%#Eval("Address") %></td>
                        ...
                        <td style="font-weight: bold; font-family: Arial;">金力同步：<%#Eval("submitgk").ToString() == "1" ? "已同步" : "未同步" %><%# string.IsNullOrEmpty(Eval("KukaiCode").ToString()) ? "" : Eval("submitkk").ToString() == "1" ? "&nbsp;/&nbsp;酷开同步：已同步" : "&nbsp;/&nbsp;酷开同步：未同步" %><br /><Hi:FormatedMoneyLabel ID="lblOrderTotal" CssClass="money" Money='<%#Eval("OrderTotal") %>' runat="server" />
                            <span class="Name">
                                <%--修改价格--%>
                        ...
                        <td style="color: #000" class="td_txt_right">&nbsp;
                            <Hi:OrderStatusLabel ID="lblOrderStatus" OrderStatusCode='<%# Eval("OrderStatus") %>'
                                runat="server" />
                            <span class="Name" style="display: <%=Hidistro.ControlPanel.Config.CustomConfigHelper.Instance.AutoShipping?"none":"" %>"><a class="link_default_blue" href='<%# "OrderDetails.aspx?OrderId="+Eval("OrderId") %>'>详情</a></span>
                        </td>
                        <td class="td_txt_cenetr">&nbsp;<span class="Name" style="text-indent: 0px;">
                            ...
                            <span class="FilialeSendOrder" style="display:none"><asp:Label CssClass="submit_faihuo" ID="lkbtnSendGoods" Visible="false" runat="server"> 
                                    <%--<a href="javascript:DialogFrame('<%# "sales/SendOrderGoods.aspx?OrderId="+ Eval("OrderId") %>&reurl='+ encodeURIComponent(goUrl),'订单发货',null,null, function () { location.href=goUrl })"><%#Eval("OrderSource").ToString() == "1" ? "" : "" %></a>--%>
                                    <%#Eval("OrderSource").ToString() == "99" ? "等待港口发货" : (Eval("OrderStatus").ToString() == "2" && Eval("submitgk").ToString() == "1" && string.IsNullOrEmpty(Eval("KukaiCode").ToString())) ? "<a href=\"javascript:DialogFrame('sales/SendOrderGoods.aspx?OrderId="+Eval("OrderId")+"&reurl=encodeURIComponent(goUrl)','订单发货',null,null, function () { location.href=goUrl })\">发货</a>" : "" %>
                            </asp:Label>
                            <br />
                            <a class="link_default_gray" name="link_default_gray" href="javascript:SendAH('<%#Eval("OrderId") %>');"><%#(Eval("OrderStatus").ToString() == "2" && Eval("submitgk").ToString() == "0") ? "推送金力" : "" %></a>
                            <a class="link_default_gray" name="link_default_gray" href="javascript:SendKK('<%#Eval("OrderId") %>');"><%#(Eval("OrderStatus").ToString() == "2" && Eval("submitgk").ToString() == "1" && Eval("submitkk").ToString() == "0" && !string.IsNullOrEmpty(Eval("KukaiCode").ToString())) ? "推送酷开" : "" %></a>
                              </span>
    ...
    <div id="showOrderSource" style="display: none;">
        <div class="frame-content">
            <p>
                <em>溯源详情</em>
            </p>
            <p>
                <span class="frame-span frame-input110">分公司名称：</span>
                <label id='fgsName'></label>
            </p>
            <p>
                <span class="frame-span frame-input110">分公司电话：</span>
                <label id='fgsCell'></label>
            </p>
            <p>
                <span class="frame-span frame-input110">门店账号：</span>
                <label id='storeDzCode'></label>
            </p>
            <p>
                <span class="frame-span frame-input110">门店名称：</span>
                <label id='storeName'></label>
            </p>
            <p>
                <span class="frame-span frame-input110">门店负责人：</span>
                <label id='storePerson'></label>
            </p>
            <p>
                <span class="frame-span frame-input110">联系电话：</span>
                <label id='storeCell'></label>
            </p>
        </div>
    </div>
    <!--编辑备注--->
...
<asp:Content ID="Content3" ContentPlaceHolderID="validateHolder" runat="server">
    ...
    <script type="text/javascript">
        //页面首次加载，分分公司用户屏蔽发货按钮
        $(function () {
            if ($("#hidFilialeOrder").val() != "") {
                $(".FilialeSendOrder").css("display", "");
                $("#txtFilialeName").hide();
            }
        })
        ...
        function ShowOrderSource(orderId) {
            $("#fgsName").html("");
            $("#fgsCell").html("");
            $("#storeDzCode").html("");
            $("#storeName").html("");
            ...
            $.ajax({
                type: "get",
                url: url,
                data: { action: 'showordersource', orderId: orderId },
                dataType: "json",
                async: false,
                success: function (data) {
                    if (data.success == "true") {
                        $("#fgsName").html(data.FgsName);
                        $("#fgsCell").html(data.FgsPhone);
                        $("#storeDzCode").html(data.AccountALLHere);
                        $("#storeName").html(data.StoreName);
                        $("#storePerson").html(data.StoreRelationPerson);
                        $("#storeCell").html(data.StoreRelationCell);

                        //DialogShow("订单来源", 'sourceframe', 'showOrderSource', 'ctl00_contentHolder_btnCloseOrder');
                        DialogShow("订单来源", 'sourceframe', 'showOrderSource');
                        //btnClose
                    } else
                        alert("操作失败，失败原因：" + data.Msg);
                }
            });
        }
        ...
        //推送金力
        function SendAH(orderId) {
            var r = confirm("确认要推送到金力吗？");
            if (r == true) {
                var url = '/API/VshopProcess.ashx';
                $.ajax({
                    type: "get",
                    url: url,
                    data: { action: 'sendah', orderId: orderId },
                    dataType: "json",
                    async: false,
                    success: function (data) {
                        if (data.success == "true") {
        ...
        //重新酷开
        function SendKK(orderId) {
            var r = confirm("确认要推送到酷开吗？");
            if (r == true) {
                var url = '/API/VshopProcess.ashx';
                $.ajax({
                    type: "get",
                    url: url,
                    data: { action: 'sendkk', orderId: orderId },
                    dataType: "json",
                    async: false,
                    success: function (data) {
                        if (data.success == "true") {
        ...
        function todayOrderPrint() {
            ...
            */
            $.ajax({
                ...
                data: "{startDate:'" + startDate + "',endDate:'" + endDate + "'}",         //这里是要传递的参数，格式为 data: "{paraName:paraValue}",下面将会看到      
                dataType: 'json',
                success: function (result) {     //回调函数，result，返回值
                    $("#printDiv").show();
                    $("#printDiv").html(result.d);
        ...
        function batchPrintData(type) {
            ...
            try {
                ...
                if (type == "dzmd") {
                    LODOP.PRINT_INIT("打印订单");
                    LODOP.SET_PRINT_PAGESIZE(3, "108mm", "185mm", "");
                    LODOP.ADD_PRINT_HTM(0, 0, 730, $("#printDiv").height(), $("#printDiv").html());
                }
                LODOP.PRINT();
            } catch (e) {
        ...
        function selectde(e) {
            ...
            $("#hidFilialeId").val(productId);//将选中值赋给隐藏域  后台取值
            $("#txtFiliale").val(e.innerHTML);
            document.getElementById("bcul").style.display = "none";
        }
    </script>
</asp:Content>
