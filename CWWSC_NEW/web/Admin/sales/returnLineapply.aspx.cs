...
{
    public partial class returnLineapply : AdminPage
    {
        ...
        private ReturnsApplyQuery GetQuery()
        {
            ...
            //2017-08-04 验证当前登陆用户类型
            bool isFiliale = false;//是否分公司
            ManagerInfo currentManager = ManagerHelper.GetCurrentManager();
            SiteSettings masterSettings = SettingsManager.GetMasterSettings(false);
            if (currentManager.RoleId == masterSettings.FilialeRoleId)
            {
                isFiliale = true;//当前登陆为供应商用户
            }
            returnsApplyQuery.OrderSource = 4;
            if (isFiliale)
                returnsApplyQuery.FilialeId = currentManager.ClientUserId;
