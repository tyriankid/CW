...
function ResetGroupBuyProducts() {
    ...
    var postUrl = "../promotion/addgroupbuy.aspx?isCallback=true&action=getGroupBuyProducts&timestamp=";
    postUrl += new Date().getTime() + "&categoryId=" + categoryId + "&sku=" + encodeURI(sku) + "&productName=" + encodeURI(productName);
    document.getElementById("ctl00_contentHolder_dropProduct").options.length = 0;
    $.ajax({
        url: postUrl,
...
function FillProducts(product) {
    var productSelector = $("#ctl00_contentHolder_dropProduct");
