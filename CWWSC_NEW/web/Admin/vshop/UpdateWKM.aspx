<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true"
    CodeBehind="UpdateWKM.aspx.cs" Inherits="Hidistro.UI.Web.Admin.vshop.UpdateWKM" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="contentHolder" runat="server" ClientIDMode="Static">
    <div class="areacolumn clearfix">
        ...
        <ul class="btn Pa_100 clearfix">
            <asp:Button ID="btnUpdateWKM" runat="server" OnClientClick="return PageIsValid();"
                Text="保 存" CssClass="submit_DAqueding" onclick="btnUpdateWKM_Click"/>
        </ul>
        </div>
    </div>
    </div>
...
<asp:Content ID="Content3" ContentPlaceHolderID="validateHolder" runat="server">
    <script type="text/javascript">
        ...
        function initSbjInfo() {
            $("#subjectArea").show();
            //根据当前题目和答案的隐藏域凑行html
            var sbjList = $("#subjectInfo").val().split(';');//题目列
            var optList = $("#optionInfo").val().split(';');//题目列

            var inputHtml = "";
            for (var i = 1; i <= sbjList.length; i++) {
                inputHtml += "<tr name='subject'>";
                inputHtml += "<td>题目" + i + "：<input type='text' id='sbj_" + i + "' value='"+sbjList[i-1]+"'  style='width:200px;height:25px'/></td>";
                var optListPerSbj = optList[i-1].split('/');
                for (var j = 1; j <= 4; j++) {
                    if (optListPerSbj[j-1]!=null)
                        inputHtml += "<td>" + j + "：<input type='text' id='option_" + i + "_" + j + "' value='" + optListPerSbj[j-1]+ "' style='width:150px;height:25px'/></td>";
                    else
                        inputHtml += "<td>" + j + "：<input type='text' id='option_" + i + "_" + j + "' style='width:150px;height:25px'/></td>";
                }
                inputHtml += "<td><input type='file' id='img_" + i + "' name='img_" + j + "' /></td>";
                inputHtml += "</tr>";
            }
