<%@ Page Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true"
    CodeBehind="ProductOnService.aspx.cs" Inherits="Hidistro.UI.Web.Admin.ProductOnService" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="contentHolder" runat="server">
        ...
        <div class="datalist">
                ...
                <ul>
                    ...
                    <li>
                        <abbr class="formselect">
                            <Hi:ProductServiceClassDropDownList ID="dropClassList" runat="server" NullToDisplay="--请选择服务品类--"
                                Width="153" />
                        </abbr>
                    </li>
                </ul>
            ...
            <div class="searcharea clearfix" style="padding: 3px 0px 10px 0px;">
                <ul>
                    ...
                    <li>
                        <%--<a href="AddProductService.aspx" class="submit_jia100">添加新服务商品</a>--%>
                        <input type="button" name="btnAdd" value="服务商品上架" id="btnAdd" onclick="gotoAddProductSerice()" class="searchbutton" />
                    </li>
                </ul>
            </div>
            </div>
            ...
            <UI:Grid runat="server" ID="grdProducts" Width="100%" AllowSorting="true" ShowOrderIcons="true"
                ...
                <Columns>
                    ...
                    <asp:TemplateField ItemStyle-Width="46%" HeaderText="商品" HeaderStyle-CssClass="td_right td_left">
                        <ItemTemplate>
                            ...
                            <div style="float: left;">
                                <span class="Name"><a href='<%#"../../Vshop/ProductServiceDetails.aspx?isbackshow=1&productId="+Eval("ProductId")%>'
                                    target="_blank">
                                        <%# Eval("ProductName") %></a></span> <span class="colorC" style="display:block">
                                        商品内码：<%# Eval("ProductCode") %>
                                        <%# string.IsNullOrEmpty(Eval("KukaiCode").ToString()) ? "" : "酷开商品内码：" + Eval("KukaiCode") %>
                                        库存：<%# Eval("Stock") %>
                    ...
                    <asp:TemplateField HeaderText="商品状态" ItemStyle-Width="80" ItemStyle-CssClass="td_txt_cenetr" >
                        <ItemTemplate>
                            <span>
                                ...
                                <%--<asp:Label ID="lableSaleStatus" runat="server" Text='<%#Eval("SaleStatus")%>'></asp:Label>--%>
                    ...
                    <asp:TemplateField HeaderText="操作" HeaderStyle-Width="95" ItemStyle-CssClass="td_txt_cenetr" >
                        <ItemTemplate>
                            <span class="submit_bianji"><a href="<%#"EditProductService.aspx?productId="+Eval("ProductId")%>&reurl=<%=LocalUrl %>">编辑</a></span>
    ...
    <asp:HiddenField ID="hidManage" runat="server" />
</asp:Content>
<asp:Content ID="Content3" ContentPlaceHolderID="validateHolder" runat="server">
    <script type="text/javascript" src="producttag.helper.js"></script>
    <script type="text/javascript">
        function gotoAddProductSerice() {
            var url = 'AddProductService.aspx?time=' + (new Date()).getTime().toString();
        ...
        $(document).ready(function () {
            ...
            ////如果是管理有不是管理员就隐藏设置批发价
            //if ($("#ctl00_contentHolder_hidManage").val() != "true") {
            //    $(".set_pfprice").hide();
            //}
        });

        //弹出不通过内容
        function ShouBackAccount(id) {
            if (id != null && id != "") {
                DialogFrame("product/ShowBackAccount.aspx?ProductId=" + id, "不通过原因", 480, 320);
                //DialogFrame("product/EditStocks.aspx?ProductIds=" + id, "调整库存", 880, null);
            }
        }
