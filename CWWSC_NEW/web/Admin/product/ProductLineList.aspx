<%@ Page Language="C#" MasterPageFile="~/Admin/Admin.Master" AutoEventWireup="true"
    CodeBehind="ProductLineList.aspx.cs" Inherits="Hidistro.UI.Web.Admin.ProductLineList" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="contentHolder" runat="server">
    <div class="dataarea mainwidth databody">
        ...
        <div class="datalist">
            ...
            <div class="functionHandleArea clearfix">
                ...
                <div class="batchHandleArea" role="ZBCZ">
                    <ul>
                        <li class="batchHandleButton"><span class="signicon"></span><span class="allSelect">
                            ...
                            <span class="delete" style="display: none">
                                <Hi:ImageLinkButton ID="btnRemove" runat="server" Text="清除代理商品区域" IsShow="true" DeleteMsg="确定要清除商品区域关系吗？" OnClientClick="SetRemove()" /></span>
            ...
            <UI:Grid runat="server" ID="grdProducts" Width="100%" AllowSorting="true" ShowOrderIcons="true"
                ...
                <Columns>
                    ...
                    <asp:TemplateField ItemStyle-Width="46%" HeaderText="商品" HeaderStyle-CssClass="td_right td_left">
                        <ItemTemplate>
                            ...
                            <div style="float: left;">
                                <span class="Name">
                                    <%# Eval("ProductName") %></span> <span class="colorC" style="display: block">商品内码：<%# Eval("ProductCode") %><span style="margin-left: 15px;">库存：<%# Eval("Stock") %></span><span style="margin-left: 15px;">成本价：<%# Eval("CostPrice", "{0:f2}")%></span></span></div>
                            </div>
                        </ItemTemplate>
                    </asp:TemplateField>
                    <asp:TemplateField ItemStyle-Width="22%" HeaderText="商品价格">
                        <ItemTemplate>
                            <span class="Name">一口价：<%# Eval("SalePrice", "{0:f2}")%></span><span style="margin-left: 15px;">市场价：<asp:Literal ID="litMarketPrice" runat="server" Text='<%#Eval("MarketPrice", "{0:f2}")%>'></asp:Literal></span>
                        </ItemTemplate>
                    </asp:TemplateField>
                    ...
                    <asp:TemplateField HeaderText="操作" HeaderStyle-Width="95" ItemStyle-CssClass="td_txt_cenetr">
                        <ItemTemplate>
                            <span class="ZB">
                             <span class="submit_shanchu"><a href="<%#"seeFilialeProductList.aspx?ProductCode="+Eval("ProductCode")%>">查看(分公司)</a></span>
                            <span class="submit_bianji"><a href="<%#"editProductLine.aspx?productId="+Eval("ProductId")%>&reurl=<%=LocalUrl %>">编辑</a></span>
                            <span class="submit_shanchu"> <Hi:ImageLinkButton ID="btnDel" CommandName="Delete" runat="server" Text="删除" IsShow="true" DeleteMsg="确定要把商品删除吗?" /></span></span>
                            <span class="submit_bianji" role="FGS" style="display: none"><span style="display: <%#SelectFilialeProductNum(Eval("ProductCode").ToString())?"none":"black"%>">商品已上架</span><a href="<%#"AddFilialeProductLine.aspx?productId="+Eval("ProductId")%>&reurl=<%=LocalUrl %>" style="display: <%#SelectFilialeProductNum(Eval("ProductCode").ToString())?"black":"none"%>">商品上架(线下)</a></span>
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
            </UI:Grid>
    ...
    <div style="display: none">
        <asp:HiddenField ID="hiUrlRoot" runat="server" />
...
<asp:Content ID="Content3" ContentPlaceHolderID="validateHolder" runat="server">
    ...
    <script type="text/javascript">
        //通过生产二维码接口，返回流文件直接下载
        function getImages(productId, productcode) {
            var root = $("#ctl00_contentHolder_hiUrlRoot").val();
            var value = root + "/vshop/ProductDetails.aspx?ProductId=" + productId;
            window.location = "/API/GetQRCode.ashx?type=1&name=" + productcode + "&code=" + escape(value);
        }
