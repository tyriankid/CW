...
$(document).ready(function () {
    ...
    neigouPriceRow = document.getElementById("neigouPriceRow");
...
function restoreState() {
    ...
    if ($("#ctl00_contentHolder_chkSkuEnabled").attr("checked") == "checked") {
        ...
        $.each(selectedSkus, function() {
            ...
            $("#neigouPrice_" + rowIndex).val($(this).attr("neigouPrice"));
...
function setCtrlDisplay(displayCssStatus) {
    ...
    neigouPriceRow.style.display = displayCssStatus;//内购价设置显示隐藏
...
function prepareSkus() {
    ...
    tableHeader.append($("<td align=\"center\">货号(型号)<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:130px\"><em >*<\/em>一口价(元)<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:130px\">内购价(元)<\/td>"));
    tableHeader.append($("<td align=\"center\" style=\"width:100px\"><em >*<\/em>结算价(元)<\/td>"));
...
function removeSkuField(skuId, skuName, showConfirm) {
    if ($(".SpecificationTable td sup").length == 1) {
        alert("最后一个规格不能删除,如果要删除全部规格,请点击关闭规格按钮！");
        return;
    }
...
function addItem() {
    ...
    dataRow.append(createNeigouPriceCell(newRowIndex));//添加内购价格
...
function createNeigouPriceCell(rowIndex) {
    var cell = createCell();
    var priceCell = $(String.format("<input type=\"text\" class=\"skuItem_NeigouPrice\" id=\"neigouPrice_{0}\" style=\"width:50px;\" \/>", rowIndex));
    $(priceCell).val($("#ctl00_contentHolder_txtNeigouPrice").val());
    cell.append(priceCell);
    return cell;
}
...
function cancelValid() {
    ...
    $("#ctl00_contentHolder_txtNeigouPrice").removeAttr("ValidateGroup");
...
function reBindValid() {
    ...
    $("#ctl00_contentHolder_txtNeigouPrice").attr("ValidateGroup", "default");
...
function doSubmit() {
    if ($("#ctl00_contentHolder_dropClassList").val() == "") {
        alert("请选择服务品类！");
        return false;
    }
    ...
    if (skuEnabled) {
        ...
        if (!checkNeigouPrice()) return false;
...
function loadSkus() {
    ...
    $.each($(".SpecificationTr"), function (i, valitem) {
        ...
        var neigouPrice = $("#neigouPrice_" + rowIndex).val();
        ...
        //var itemXml = String.format("<item skuCode=\"{0}\" salePrice=\"{1}\" neigouPrice=\"{2}\" costPrice=\"{3}\" purchasePrice=\"{4}\" qty=\"{5}\" alertQty=\"{6}\" weight=\"{7}\">", skuCode, salePrice, neigouPrice, costPrice, purchasePrice, qty, alertQty, weight);
        var itemXml = String.format("<item skuCode=\"{0}\" salePrice=\"{1}\" neigouPrice=\"{2}\" costPrice=\"{3}\" purchasePrice=\"{4}\" qty=\"{5}\" alertQty=\"{6}\">", skuCode, salePrice, neigouPrice, costPrice, purchasePrice, qty, alertQty);
        itemXml += "<skuFields>";
        for (i = 0; i < cellFields.length; i++) {
...
function checkNeigouPrice() {
    return checkPrice($(".skuItem_NeigouPrice"), false, "内购价");
}
