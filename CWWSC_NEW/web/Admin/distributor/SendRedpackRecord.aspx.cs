    ...
    public partial class SendRedpackRecord : AdminPage
    {
        ...
        protected void btnSendAllRedPack_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    str = this.SendRedPack(notSendRedpackRecord.Rows[i]["OpenID"].ToString(), "", notSendRedpackRecord.Rows[i]["Wishing"].ToString(), notSendRedpackRecord.Rows[i]["ActName"].ToString(), "分销商发红包提现", int.Parse(notSendRedpackRecord.Rows[i]["Amount"].ToString()), int.Parse(notSendRedpackRecord.Rows[i]["ID"].ToString()), Boolean.Parse(notSendRedpackRecord.Rows[i]["IsSend"].ToString()));
                    if (str != "1")
                    {
                        empty = string.Empty;
                        if (num <= 0)
                    ...
                    int amount = 0;
                    int userid = 0;
                    if (int.TryParse(notSendRedpackRecord.Rows[i]["ID"].ToString(), out redid) && int.TryParse(notSendRedpackRecord.Rows[i]["Amount"].ToString(), out amount) && int.TryParse(notSendRedpackRecord.Rows[i]["UserID"].ToString(), out userid))
                    {
                        DistributorsBrower.SetRedpackRecordIsUsed(redid, true);
                        decimal num1 = decimal.Parse(amount.ToString()) / new decimal(100);
                        VShopHelper.UpdateBalanceDistributors(userid, num1);
                    }
                }
            }
            //if (str == "1")
        ...
        public string SendRedPack(string re_openid, string sub_mch_id, string wishing, string act_name, string remark, int amount, int sendredpackrecordid, bool isSend)
        {
